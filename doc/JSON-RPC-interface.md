...
The headless daemon `trollcoind` has the JSON-RPC API enabled by default, the GUI
`trollcoin-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by trollcoin-cli when a `-rpcwallet=` parameter is passed in.
...
```sh
# Get block count from the / endpoint when rpcuser=alice and rpcport=17000
$ curl --user alice --data-binary '{"jsonrpc": "1.0", "id": "0", "method": "getblockcount", "params": []}' -H 'content-type: text/plain;' localhost:17000/

# Get balance from the /wallet/walletname endpoint when rpcuser=alice, rpcport=17000 and rpcwallet=desc-wallet
$ curl --user alice --data-binary '{"jsonrpc": "1.0", "id": "0", "method": "getbalance", "params": []}' -H 'content-type: text/plain;' localhost:17000/wallet/desc-wallet
...
trollcoin-cli createwallet mywallet false false "" false true true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
trollcoin-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
trollcoin-cli -named createwallet mywallet load_on_startup=true
```

## Versioning

The RPC interface might change from one major version of TrollCoin to the
...
The RPC interface allows other programs to control TrollCoin,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure TrollCoin to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running TrollCoin can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your TrollCoin
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use TrollCoin for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  an RPC interface on the same port as used by TrollCoin in order to
  trick you into revealing your authentication credentials.  For this
  reason, it is important to only use TrollCoin for
  security-sensitive operations on a computer whose other programs you
  trust.

- **Securing remote network access:** You may optionally allow other
  computers to remotely control TrollCoin by setting the `rpcallowip`
  ...
  Internet.**  Although TrollCoin's RPC interface does use
  ...
  `trollcoind -help` for more information about these settings and other
  settings described in this document.

    Related, if you use TrollCoin inside a Docker container, you may
    need to expose the RPC port to the host system.  The default way to
    do this in Docker also exposes the port to the public Internet.
    Instead, expose it only on the host system's localhost, for example:
    `-p 127.0.0.1:17000:17000`

- **Secure authentication:** By default, TrollCoin generates unique
  login credentials each time it restarts and puts them into a file
  readable only by the user that started TrollCoin, allowing any of
  that user's RPC clients with read access to the file to login
  automatically.  The file is `.cookie` in the TrollCoin
  configuration directory, and using these credentials is the preferred
  RPC authentication method.  If you need to generate static login
  credentials for your programs, you can use the script in the
  `share/rpcauth` directory in the TrollCoin source tree.  As a final
...
transactions that are considered eligible for inclusion in a block by the node
at the time of the RPC.
...
example, a wallet transaction that was removed from the mempool prior to
