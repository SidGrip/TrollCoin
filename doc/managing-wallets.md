...
TrollCoin does not create a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `trollcoin-cli help createwallet`.

```
$ trollcoin-cli createwallet "wallet-01"
```
...
| -----------------|:---------------------------------------------------------------|
| Linux            | `/home/<user>/.trollcoin/wallets`                              |
| Windows          | `C:\Users\<user>\AppData\Roaming\TrollCoin\wallets`            |
| macOS            | `/Users/<user>/Library/Application Support/TrollCoin/wallets`  |
...
```
$ trollcoin-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ trollcoin-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ trollcoin-cli -named createwallet wallet_name="wallet-01" passphrase="passphrase"
```
...
```
$ trollcoin-cli -rpcwallet="wallet-01" sendtoaddress "<address>" 0.01
error code: -13
error message:
...
This command takes the passphrase and an argument called `timeout`, which specifies the time in seconds that the wallet decryption key is stored in memory. After this period expires, the user needs to execute this RPC again. An optional third argument, `stakingonly`, unlocks the wallet for staking only; sending coins still requires a full unlock.

```
$ trollcoin-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```

In the GUI, the wallet can be unlocked with `Settings` -> `Unlock Wallet…`. The unlock dialog has a `For staking only` checkbox, checked by default, which unlocks the wallet for staking without allowing coins to be sent; `Settings` -> `Lock Wallet` locks it again. When the user sends coins, the passphrase will be prompted automatically.
...
```
$ trollcoin-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
Bitcoin Core [version 0.13](https://github.com/bitcoin/bitcoin/blob/master/doc/release-notes/release-notes-0.13.0.md) introduced HD wallets with deterministic key derivation, which TrollCoin inherits. With HD wallets, users no longer lose funds when restoring old backups because all addresses are derived from the HD wallet seed.
...
Non-HD legacy wallets must be backed up every 100 keys used since the previous backup, or even more often to maintain the metadata.
...
```
$ trollcoin-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ trollcoin-cli -rpcwallet="restored-wallet" getwalletinfo
```
...
Migration can also be started in the GUI via `File` -> `Migrate Wallet`, which is enabled when a
legacy wallet is loaded.
...
unexpectedly or otherwise misses any scripts, please create an issue on the
[issue tracker](https://github.com/SidGrip/TrollCoin/issues). A backup of the
