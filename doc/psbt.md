# PSBT Howto for TrollCoin

TrollCoin provides an RPC interface for Partially Signed Bitcoin
...
PSBT is an interchange format for transactions that are not fully signed
...
Overall, the construction of a fully signed transaction goes through the
following steps:
...
- An **Extractor** produces a valid transaction (in network format)
...
- **`converttopsbt` (Creator)** is a utility RPC that converts an
  unsigned raw transaction to PSBT format. By default it fails if any
  signatures are already present; setting `permitsigdata` discards them.
...
- **`analyzepsbt`** is a utility RPC that examines a PSBT and reports the
  ...
  final vsize and feerate.


### Workflows

#### Multisig with multiple TrollCoin instances
...
Note that creating a new legacy wallet requires starting the daemon with
`-deprecatedrpc=create_bdb`.

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
TrollCoin. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `trollcoin-cli` is used.
...
Later, when *V* TROLL has been received on *Amulti*, and Bob and Carol want to
