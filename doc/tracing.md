# User-space, Statically Defined Tracing (USDT) for TrollCoin

TrollCoin includes statically defined tracepoints to allow for more
...
5. SigOps in the Block (excluding coinbase SigOps) as `int64`
...
**Note**: TrollCoin uses temporary clones of the _main_ UTXO cache
...
6. If the coin is a coinstake as `bool`
...
Multiple tools can list the available tracepoints in a `trollcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/trollcoind
stap net        outbound_message 0x0000000000107c05 /src/trollcoind
stap validation block_connected  0x00000000002fb10c /src/trollcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in `trollcoind`.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/trollcoind | grep NT_STAPSDT -A 4 -B 2
