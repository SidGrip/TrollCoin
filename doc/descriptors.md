# Support for Output Descriptors in TrollCoin

TrollCoin supports Output Descriptors, inherited from Bitcoin Core. This is a
...
- P2PKH addresses (base58, of the form `T...` for mainnet or `[nm]...` for testnet/regtest). Note that P2PKH addresses in descriptors cannot be used for P2PK outputs (use the `pk` function instead).
- P2SH addresses (base58, of the form `3...` for mainnet or `2...` for testnet/regtest, defined in [BIP 13](https://github.com/bitcoin/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32 and bech32m, of the form `ttroll1...` for testnet or `trollrt1...` for regtest, defined in [BIP 173](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki) and [BIP 350](https://github.com/bitcoin/bips/blob/master/bip-0350.mediawiki)). SegWit is not active on TrollCoin mainnet, so `troll1...` addresses cannot be used there.
...
wallets and PSBTs, as well as a signing flow, see the [multisig tutorial](multisig-tutorial.md).
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a TrollCoin descriptor wallet to serve as your
...
existing legacy wallets, a convenience function `combo` is
...
All RPCs in TrollCoin will include the checksum in their output. Only
