...
The data directory is the default location where the TrollCoin files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.trollcoin/`
macOS    | `$HOME/Library/Application Support/TrollCoin/`
Windows  | `%APPDATA%\TrollCoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `trollcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual TrollCoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `trollcoin.conf`      | User-defined [configuration settings](trollcoin-conf.md) for `trollcoind` or `trollcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `trollcoind.pid`      | Stores the process ID (PID) of `trollcoind` or `trollcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `trollcoind` or `trollcoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [trollcoin.conf](trollcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`trollcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the codebase (versions and pull requests refer to upstream Bitcoin Core, from which this codebase is derived):
