# CJDNS support in TrollCoin

It is possible to run TrollCoin over CJDNS, an encrypted IPv6 network that
...
done outside of TrollCoin, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the TrollCoin network and individual nodes.
...
before it will work with your TrollCoin node. This is described in steps
...
## Run TrollCoin with CJDNS

Once you are connected to the CJDNS network, the following TrollCoin
...
When enabled, this option tells TrollCoin that it is running in an
...
IPv6 local network. This helps TrollCoin perform better address management:
...
`trollcoin-cli -addrinfo` to see the number of CJDNS addresses known to your
node.
...
## CJDNS-related information in TrollCoin

There are several ways to see your CJDNS address in TrollCoin:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `trollcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `trollcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `trollcoin-cli help getnodeaddresses` for details.
