...
The purpose of this library is to make the script verification functionality that is critical to TrollCoin's consensus available to other applications, e.g. to language bindings. The library and its header keep their upstream Bitcoin Core name, `bitcoinconsensus`.
...
- `const unsigned char *txTo` - The transaction with the input that is spending the previous output.
- `unsigned int txToLen` - The number of bytes for the `txTo`.
- `UTXO *spentOutputs` - Previous outputs spent in the transaction. `UTXO` is a struct composed by `const unsigned char *scriptPubKey`, `unsigned int scriptPubKeySize` (the number of bytes for the `scriptPubKey`) and `int64_t value`.
- `unsigned int spentOutputsLen` - The number of outputs in `spentOutputs`. Must match the number of inputs in `txTo`.
- `unsigned int nIn` - The index of the input in `txTo` that spends the `scriptPubKey`.
- `unsigned int flags` - The script validation flags *(see below)*.
- `bitcoinconsensus_error* err` - Will have the error/success code for the operation *(see below)*.
...
- `bitcoinconsensus_ERR_TX_DESERIALIZE` - An error deserializing `txTo`
