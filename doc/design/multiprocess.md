# Multiprocess TrollCoin

On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `trollcoin-node`, `trollcoin-wallet`, and `trollcoin-gui` executables alongside existing `trollcoind` and `trollcoin-qt` executables.

`trollcoin-node` is a drop-in replacement for `trollcoind`, and `trollcoin-gui` is a drop-in replacement for `trollcoin-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after [#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `trollcoin-gui` will spawn a `trollcoin-node` process to run P2P and RPC code, communicating with it across a socket pair, and `trollcoin-node` will spawn `trollcoin-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after [#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `trollcoin-node`, `trollcoin-wallet`, and `trollcoin-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `trollcoin-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
```
cd <TROLLCOIN_SOURCE_DIRECTORY>
make -C depends NO_QT=1 MULTIPROCESS=1
CONFIG_SITE=$PWD/depends/x86_64-pc-linux-gnu/share/config.site ./configure
make
src/trollcoin-node -regtest -printtoconsole -debug=ipc
```
