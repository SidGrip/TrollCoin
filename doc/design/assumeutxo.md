...
Assumeutxo is a feature that allows fast bootstrapping of a validating trollcoind
instance.
...
(`m_assumeutxo_data` in `src/kernel/chainparams.cpp`). No snapshot hashes are
compiled in for mainnet or testnet yet, so snapshots can currently only be
loaded on regtest.
...
To save space, it's possible to delete the
snapshot file as soon as `loadtxoutset` finishes.

While the snapshot is being loaded, nearly all of the configured `-dbcache` is
temporarily allocated to the snapshot chainstate to speed up the bulk load;
caches are rebalanced once activation completes (see `MaybeRebalanceCaches()`).

As the background sync continues there will be temporarily two chainstate
directories, each roughly the size of the full UTXO set (likely larger than
...
`m_from_snapshot_blockhash` is unset. This chainstate is (maybe obviously)
considered active. This is the "traditional" mode of operation for trollcoind.
...
### Trollcoind restarts sometime after snapshot validation has completed
