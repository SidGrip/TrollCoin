...
TrollCoin uses [Doxygen](https://www.doxygen.nl/) to generate its developer documentation.
...
configuration options and toggled while trollcoind is running with the `logging`
RPC.  For instance, launching trollcoind with `-debug` or `-debug=1` will turn on
...
you can run with the `-testnet` config option to test with "play TROLL" on a
test network.
...
TrollCoin is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run trollcoind.

You can then use the `-debug=lock` configuration option at trollcoind startup or
`trollcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `trollcoin-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_trollcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_trollcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/trollcoind -printtoconsole
```
...
# A unit test coverage report will now be accessible at
# `./test_trollcoin.coverage/index.html`.
```

`make cov` also attempts a combined report (`total.coverage`) that additionally
requires the functional test suite; in a checkout without it, the run stops with
an error after the unit test report has been generated.

### Performance profiling with perf

Profiling is a good way to get a precise idea of where time is being spent in
code. One tool for doing profiling on Linux platforms is called
[`perf`](https://www.brendangregg.com/perf.html). Perf can observe a running
...
To profile a running trollcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep trollcoind` -- sleep 60
```
...
TrollCoin can be compiled with various "sanitizers" enabled, which add
...
See the upstream documentation for more information about any additional options.

There are a number of known problems when using the `address` sanitizer. The
address sanitizer is known to fail in
[sha256_sse4::Transform](/src/crypto/sha256_sse4.cpp) which makes it unusable
unless you also use `--disable-asm` when running configure. Please send pull
requests if you can fix any errors found by the address sanitizer (or any other
sanitizer).
...
- Stake minter thread (`b-trollcoin-stake-miner-<wallet>`)
  : One thread per staking wallet; creates and signs proof-of-stake blocks.
...
- Net threads:
  ...
  - ThreadSocketHandler (`b-net`)
    : Sends/Receives data from peers on port 15000.
...
However, in open source software such as TrollCoin, where everyone uses
...
pay attention to for reviewers of TrollCoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: The codebase uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use `.c_str()` sparingly. Its only valid use is to pass C++ strings to C functions that take NULL-terminated
  ...
  - In cases where do you call `.c_str()`, you might want to additionally check that the string does not contain embedded '\0' characters, because
    ...
    and reject it if there are any (see `ParsePrechecks` in `src/univalue/lib/univalue_get.cpp` for an example).
...
Changes to these should be sent upstream rather than PRed directly against
this project; they will be picked up in a future subtree merge.
...
file descriptors, it will cause problems with the node's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to our
...
scripted-diff commits. The bash script is included in the commit message so that reviewers can verify that
re-running the script reproduces the commit exactly. It is also helpful for rebasing (since the same script can
just be re-run on the new master commit).
...
All `release-notes*` files are merged into the published release notes
...
- Use the JSON parser for parsing, don't manually parse integers or strings from
  ...
  - *Exception*: AmountFromValue can parse amounts as string. This was introduced because many JSON
    ...
    inputting or outputting monetary values. The only exception to this is
    `getblocktemplate` because its interface is specified as-is in BIP22.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `trollcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `trollcoin-cli` and the GUI debug console use this table to determine how to
