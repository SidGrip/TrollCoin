...
Currently, it is possible to create a multisig wallet using TrollCoin only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](descriptors.md#multisig), this tutorial uses the regtest network and explains some functions in more detail.
...
Before starting this tutorial, start the TrollCoin node on the regtest network.

```bash
./src/trollcoind -regtest -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in TrollCoin. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/trollcoin-cli -regtest createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, TrollCoin single-sig wallets on testnet and regtest are created using path `m/44h/1h/0h` for PKH, `m/84h/1h/0h` for WPKH, `m/49h/1h/0h` for P2WPKH-nested-in-P2SH and `m/86h/1h/0h` for P2TR based accounts (mainnet uses coin type `10h`). Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.

```
wpkh([1c38cb11/84h/1h/0h]xpub.../0/*)#checksum

wpkh([1c38cb11/84h/1h/0h]xpub.../1/*)#checksum
```
...
All RPCs in TrollCoin will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/trollcoin-cli -regtest -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/trollcoin-cli -regtest -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
As previously mentioned, this step extracts the `m/84h/1h/0h` account instead of the path defined in [BIP 45](https://github.com/bitcoin/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/bitcoin/bips/blob/master/bip-0087.mediawiki), since there is no way to extract a specific path in TrollCoin at the time of writing.
...
external_desc_sum=$(./src/trollcoin-cli -regtest getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/trollcoin-cli -regtest getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/trollcoin-cli -regtest help importdescriptors`.
...
```bash
./src/trollcoin-cli -regtest -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./src/trollcoin-cli -regtest -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/trollcoin-cli -regtest -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/trollcoin-cli -regtest loadwallet "participant_${n}"; done
```

### 1.4 Fund the wallet

On regtest, the wallet can be funded by generating blocks to one of its addresses. Block rewards on regtest mature after 10 blocks, so generate a few extra blocks on top to make the first rewards spendable.

The default address type in TrollCoin is `legacy`, but this wallet only contains `wsh` descriptors, so the `bech32` address type must be requested explicitly.

```bash
receiving_address=$(./src/trollcoin-cli -regtest -rpcwallet="multisig_wallet_01" getnewaddress "" bech32)

./src/trollcoin-cli -regtest generatetoaddress 20 $receiving_address
```

The `getbalances` RPC may be used to check the balance. Coins with `trusted` status can be spent.

```bash
./src/trollcoin-cli -regtest -rpcwallet="multisig_wallet_01" getbalances
```
...
PSBT is a data format that allows wallets and other tools to exchange information about a transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid TrollCoin address.
...
The default change type in TrollCoin is `legacy`, so `change_type` is set to `bech32` — this wallet can only derive change from its internal `wsh` descriptor.

```bash
balance=$(./src/trollcoin-cli -regtest -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/trollcoin-cli -regtest -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/trollcoin-cli -regtest -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" options="{\"change_type\": \"bech32\"}" | jq -r '.psbt')
```
...
```bash
./src/trollcoin-cli -regtest decodepsbt $funded_psbt

./src/trollcoin-cli -regtest analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/trollcoin-cli -regtest -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/trollcoin-cli -regtest -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/trollcoin-cli -regtest combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/trollcoin-cli -regtest finalizepsbt $combined_psbt | jq -r '.hex')

./src/trollcoin-cli -regtest sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/trollcoin-cli -regtest -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/trollcoin-cli -regtest -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/trollcoin-cli -regtest finalizepsbt $psbt_2 | jq -r '.hex')
