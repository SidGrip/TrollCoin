When TrollCoin automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no TrollCoin node is listening,
or one where a service is listening that is not related to the TrollCoin network.
As a result, this service may occasionally get connection attempts from TrollCoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by TrollCoin,
trying to connect because it thinks there is a TrollCoin node on that
...
Below is a list of "bad" ports which TrollCoin avoids when choosing a peer to
