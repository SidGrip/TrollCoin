...
bool CheckSignatureEncoding(const std::vector<unsigned char> &vchSig, unsigned int flags, ScriptError* serror) {
    ...
    // TrollCoin (legacy FIX_HASHTYPE) does not reject undefined sighash types.
    return true;
}
...
public:
    ...
    void Serialize(S &s) const {
        ...
        // TrollCoin serializes nTime for every transaction version, so every
        // signature version must commit to the exact serialized value.
...
bool SignatureHashSchnorr(uint256& hash_out, ScriptExecutionData& execdata, const T& tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, const PrecomputedTransactionData& cache, MissingDataBehavior mdb)
{
    ...
    ss << tx_to.nVersion;
    // TrollCoin transactions always serialize nTime. Commit to it here to
    // prevent third-party mutation and to match the legacy and BIP143 paths.
    // This intentionally diverges from upstream BIP341, which has no nTime.
    ss << tx_to.nTime;
    ss << tx_to.nLockTime;
