...
/** Consecutive header batches reaching past the allowed lead before DoS score */
static const int MAX_HEADERS_TOO_FAR_AHEAD = 5;
...
struct Peer {
    ...
    /** Length of current streak of headers reaching further past our validated
     *  tip than elapsed time allows. Atomic because block validation reports
     *  results from its own thread. Reset as soon as the peer sends headers we
     *  can use, so only a sustained streak costs the connection. */
    std::atomic<int> m_headers_too_far_ahead{0};
...
public:
    ...
    bool updateState(NodeId peerId, BlockValidationState& state, bool ret)
    {
        ...
        if(banNode)
        {
            // Flag the peer for discouragement/disconnection if it is spamming
            // headers. The actual disconnect happens later via
...
bool PeerManagerImpl::ProcessNetBlockHeaders(CNode& pfrom, const std::vector<CBlockHeader>& block, bool min_pow_checked, BlockValidationState& state, bool old_client, const CBlockIndex** ppindex)
{
    ...
    if (gArgs.GetBoolArg("-headerspamfilter", DEFAULT_HEADER_SPAM_FILTER))
    {
        ...
        {
            LOCK(cs_main);
            // Key the per-peer accounting on the peer's real address. It used
            // to use GetAddrLocal(), which is addrMe out of the peer's own
            // version message -- data the peer chooses. That let a sender pick
            // its own bucket: rotate addrMe per connection and the thresholds
            // are never reached, or collide with the bucket honest peers land
            // in and they wear the discouragement. Cleanup keys on the real
            // address, so buckets created under a spoofed one were never
            // reclaimed either.
                                              ...
                                              bool via_compact_block, const std::string& message)
{
    ...
    case BlockValidationResult::BLOCK_HEADER_LOW_WORK:
        ...
        // too little work. That on its own is not misbehaviour.
        //
        // A chain reaching further past our validated tip than elapsed time
        // could have produced is different: a stake header proves nothing until
        // its block arrives, so this is the one case where a peer can invent
        // chain cheaply. A node whose own clock lags computes a smaller
        // allowance and would see honest headers the same way, so a single
        // occurrence is ignored and only a sustained streak is scored.
        if (state.GetRejectReason() == "header-too-far-ahead" && peer) {
            if (++peer->m_headers_too_far_ahead >= MAX_HEADERS_TOO_FAR_AHEAD) {
                peer->m_headers_too_far_ahead = 0;
...
bool PeerManagerImpl::BlockRequestAllowed(const CBlockIndex* pindex)
{
    ...
    // TrollCoin drops the GetBlockProofEquivalentTime (proof-of-work) heuristic: on a PoS
    // chain the work-equivalent-time estimate is unreliable and misfires for recent stale
    // blocks (deeming them far too old, which would disconnect a peer making a legitimate
    // filter/block request). Any block recent enough to pass the wall-clock
    // STALE_RELAY_AGE_LIMIT bound below is from the post-PoW era regardless.
...
{
    // Stage B: pre-compute this batch's scrypt hashes in parallel before the serial
    // code below reads them. Gated on an in-progress sync with this peer so garbage
    // batches still short-circuit cheaply (CheckHeadersAreContinuous) instead of
    // hashing all 2000 first. Node-local, no consensus effect.
    {
        LOCK(peer.m_headers_sync_mutex);
        if (peer.m_headers_sync) PrecomputeHeaderPoWHashes(headers);
    }
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    // Usable headers clear the too-far-ahead streak, so only a peer that keeps
    // reaching past the allowance is ever discouraged.
    peer.m_headers_too_far_ahead = 0;
