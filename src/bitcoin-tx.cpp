...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " trollcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  trollcoin-tx [options] <hex-tx> [commands]  Update hex-encoded trollcoin transaction\n"
                "or:     trollcoin-tx [options] -create [commands]   Create hex-encoded trollcoin transaction\n"
                "\n";
