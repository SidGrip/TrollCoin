...
// On x86-64 SSE2 is part of the baseline ISA, so always use the vectorized
// scrypt there. It is ~6x faster (full-chain validation scrypt-hashes every
// block header) and produces byte-identical output to the generic path.
// Other architectures fall back to generic scrypt.
#if (defined(__x86_64__) || defined(_M_X64) || defined(__amd64__)) && !defined(USE_SSE2)
#define USE_SSE2 1
#endif
...
// FreeBSD and macOS already provide le32enc/le32dec in <sys/endian.h>; use
// those (byte-identical) so the scrypt block hash is unchanged across platforms.
