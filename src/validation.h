...
/** Maximum number of worker threads for parallel header-hash pre-warming */
static const int MAX_PARALLEL_HEADER_CHECKS = 15;
/** -parallelheaderchecks default (0 = auto: number of cores, capped) */
static const int DEFAULT_PARALLEL_HEADER_CHECKS = 0;
...
/** Header-hash pre-warm thread count, resolved once at init from -parallelheaderchecks
 *  (1 = serial/off). Node-local; never affects consensus. */
extern int g_parallel_header_check_threads;

/** Fill a header batch's scrypt-hash cache in parallel so the serial sync code reads
 *  memoized values. Node-local; no-op when threads<=1 or the batch is small. */
