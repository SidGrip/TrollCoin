...
/** Keep v2 nonstandard until CSV/BIP68 is active on the selected chain. */
static constexpr decltype(CTransaction::nVersion) TX_MAX_STANDARD_VERSION_PRE_CSV{1};

/**
* Check for standard transaction types
* @return True if all outputs (scriptPubKeys) use only standard transaction forms
*/
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes,
                  bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason,
                  const bool witnessEnabled = false,
                  const decltype(CTransaction::nVersion) max_standard_version = TX_MAX_STANDARD_VERSION);
/**
* Check for standard transaction types
