...
int64_t FutureDrift(Chainstate& active_chainstate, int64_t nTime)
{
    ...
    // TrollCoin clock drift: POW_DRIFT=600s, POS_DRIFT=640s (legacy main.h). It never adopted
    // Blackcoin's V2 15s tightening, so the live chain mints blocks up to 640s ahead. Use 640s
    // so we accept every block legacy nodes produce (PoW ended at 7,777,777, so the 600/640 split
    // no longer matters; past-dated historical blocks never reach this future check).
    return nTime + 640;
}
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Consensus and policy use the exact serialized transaction time. In
    // particular, nTime == 0 must not depend on this node's wall clock.
    const int64_t nTimeTx = tx.nTime;
    ...
    const bool csvEnabled = DeploymentActiveAfter(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, Consensus::DEPLOYMENT_CSV);
    ...
    std::string reason;
    const auto max_standard_version{csvEnabled ? TX_MAX_STANDARD_VERSION : TX_MAX_STANDARD_VERSION_PRE_CSV};
    if (m_pool.m_require_standard && !IsStandardTx(tx, m_pool.m_max_datacarrier_bytes, m_pool.m_permit_bare_multisig,
                                                   m_pool.m_dust_relay_feerate, reason, witnessEnabled,
...
// TrollCoin scrypt proof-of-work emission schedule (PoW ends at nLastPOWBlock = 7,777,777).
// TrollCoin computes the subsidy from pindexBest->nHeight (the chain tip at the moment the block is
// created/connected), so block N is paid the schedule value for height N-1 — e.g. block 1 mints the
// 500,000,000 TROLL v1 swap allocation (the height-0 branch).
CAmount GetProofOfWorkSubsidy(int nHeight)
{
    int h = nHeight - 1;
    if (h <= 0)        return 500000000 * COIN; // block 1: TrollCoin v1 swap allocation
    if (h <= 777776)  return 125 * COIN;
    if (h <= 1555553) return 60 * COIN;
    if (h <= 2333330) return 34 * COIN;
    if (h <= 3111107) return 20 * COIN;
    if (h <= 3888884) return 17 * COIN;
    if (h <= 4666662) return 15 * COIN;
    if (h <= 5444438) return 12 * COIN;
    if (h <= 6222215) return 10 * COIN;
    if (h <= 6999992) return 8 * COIN;
    if (h <= 7777778) return 7 * COIN;
    return 125 * COIN; // unreached: PoW stops at nLastPOWBlock
}

CAmount GetProofOfStakeSubsidy()
{
    return COIN * 7; // flat 7 TROLL per stake block
}
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    */
    // These base flags reproduce the chain-wide settings proven by the ordered
    // historical replay.
    uint32_t flags{SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC |
                   SCRIPT_VERIFY_NULLDUMMY | SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY};

    // Low-S is enforced from LowSHeight, which tracks where legacy 2.x began
    // checking signatures at all: it skips verification while its tip is below
    // its highest hardcoded checkpoint, and enforces low-S through STRICTENC
    // above it. Below that height nothing ever checked, so the history there may
    // legitimately carry high-S signatures and must not be rejected now.
    //
    // Deliberately set before the exception lookup below, which assigns the flag
    // word wholesale rather than masking into it. A block named in
    // script_flag_exceptions has to be able to opt out of this rule, and that is
    // the only remedy available if a historical block ever needs one.
    //
    // Anyone writing such an exception needs to know exactly how much that word
    // then decides. Only the flags re-applied *below* the lookup survive it, and
    // on mainnet that is just CHECKLOCKTIMEVERIFY and NULLDUMMY, because
    // IsProtocolV3 is true for every block there (nProtocolV3Time is 0). The
    // deployment flags are not re-applied at all, since SegWit, DERSIG, CSV and
    // Taproot are parked at INT_MAX. So an exception word is the last word on
    // P2SH, STRICTENC and this rule -- and on witness and taproot enforcement
    // too -- and must spell out every flag the block still needs, not just the
    // one it is being excused from.
    if (block_index.nHeight >= consensusparams.LowSHeight) {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake() && params.GetConsensus().IsProtocolV3(block.GetBlockTime()) && !CheckProofOfStake(pindex->pprev, *block.vtx[1], block.nBits, state, view, block.vtx[1]->nTime)) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // regular-tx fees are already accumulated above via CheckTxInputs (txfee)
    ...
    TRACE6(validation, block_connected,
        ...
        nInputs,
        nSigOpsCost,
        time_5 - time_start // steady-clock duration ticks (nanoseconds on Linux)
    );
...
//! Check a header, and say whether a false return means only that the caller's
//! work-or-stake guess was unproven.
//!
//! A header from a pre-70016 peer carries no PoS marker, so AcceptBlockHeader has to
//! guess the proof type and retry the other way. That retry must fire on the failed
//! guess and on nothing else: it used to fire on any failure, so an early but otherwise
//! valid work header was reclassified as stake and refused as "bad-diffbits, incorrect
//! proof-of-stake" -- a wrong reason and a 100 score for a clock difference.
//!
//! `proof_type_unproven` is set only on the unproven-guess path. Anything added here
//! that returns false must either set state or set this flag, never neither: a new
//! silent-false branch would be reclassified exactly as the timestamp check was.
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, Chainstate& chainstate, bool fCheckPOW = true, bool fOldClient = false, bool* proof_type_unproven = nullptr)
{
    if (proof_type_unproven) *proof_type_unproven = false;

    // Check block version. TrollCoin blocks are version 1 (CBlock CURRENT_VERSION = 1) and the
    // legacy client rejects any block with nVersion > 1; it never bumped to Blackcoin's v7+ BPoS.
    if (block.nVersion > 1)
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams)) {
        if (fOldClient) {
            // The guess, not the header, is what failed here. Note that an old client's
            // genuinely unsolved work header also lands on this branch, so it is retried
            // as stake and reported as bad-diffbits. That is a wrong reason on already
            // invalid data rather than a split risk, and narrowing it further would cost
            // the retry that every legitimate 2.1.x stake header depends on.
            if (proof_type_unproven) *proof_type_unproven = true;
            return false;
        }
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of work failed");
    }

    // Check timestamp. A block that is ahead of our clock is not invalid, it is early:
    // the same header becomes acceptable once the clock catches up, which is why the
    // MAX_FUTURE_BLOCK_TIME gate in ContextualCheckBlockHeader reports BLOCK_TIME_FUTURE.
    // Report it identically here. Legacy 2.1.x refuses the block with a bare error() and
    // no DoS score at all (main.cpp:2056), so scoring the peer 100 for a clock difference
    // would evict the majority of the network over a few minutes of drift.
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, Chainstate& chainstate, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Do not restore the legacy-source coinbase FutureDrift check globally.
    // The ordered historical reconciliation removed the inherited equivalent
    // after it rejected early canonical blocks. Exact exceptions or an
    // activation boundary require a new full-history proof.

    if (block.IsProofOfStake()) {
        // Reassert the full-block PoS classification before applying the
        // remaining coinstake rules.
        ...
        // The serialized coinstake timestamp is consensus data, including zero.
        ...
        // The remaining transactions must not be coinstake.
    ...
    for (const auto& tx : block.vtx) {
        ...
        // Check transaction timestamp (TrollCoin: block time must be >= tx nTime)
...
int g_parallel_header_check_threads = 1;

void PrecomputeHeaderPoWHashes(const std::vector<CBlockHeader>& headers)
{
    const int nthreads = g_parallel_header_check_threads;
    const size_t n = headers.size();

    // Fill the batch's scrypt-hash cache on N worker threads so the serial sync code
    // reads memoized values. Skipped for small/serial batches (lazy cache still fills).
    static constexpr size_t MIN_PER_WORKER = 64;
    if (nthreads <= 1 || n < 2 * MIN_PER_WORKER) return;

    const int workers = std::min<int>(nthreads, static_cast<int>(n / MIN_PER_WORKER));
    const size_t chunk = (n + workers - 1) / workers;

    // Contiguous non-overlapping slices: one thread per header, so cache fills are race-free.
    auto work = [&headers, n](size_t begin, size_t end) {
        for (size_t i = begin; i < end && i < n; ++i) headers[i].GetPoWHash();
    };
    ...
    try {
        for (int w = 1; w < workers; ++w) {
            ...
            if (begin >= n) break;
            pool.emplace_back(work, begin, begin + chunk);
        }
    } catch (const std::system_error&) {
        // Thread creation failed: join what started (never destroy a joinable thread)
        // and bail; the lazy per-header cache still fills the rest serially.
        for (auto& t : pool) { if (t.joinable()) t.join(); }
        return;
    }
    work(0, chunk);              // this thread handles the first slice
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, NodeClock::time_point now, CChain& chain, bool fProofOfStake) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    //
    // Scored as a sync-level refusal, for the same reason as the
    // sync-checkpoint refusal below. nMaxReorganizationDepth is 500, the same
    // depth at which the checkpoint refuses, and this test runs first — so
    // scoring it BLOCK_INVALID_HEADER meant a peer offering a fork that deep
    // was banned outright at 100 points before the checkpoint's lighter score
    // could ever apply. The legacy client refuses the same fork with a plain
    // error() and no DoS score at all, and it has no equivalent of this gate,
    // so the ban was ours alone. Healing a netsplit is exactly when honest
    // majority peers offer such forks, and banning them then is the worst
    // possible moment. The fork is still refused; only the cost to the peer
    // changes.
    if (chain.Height() - nHeight >= consensusParams.nMaxReorganizationDepth) {
        ...
        return state.Invalid(BlockValidationResult::BLOCK_HEADER_SYNC, "older-than-maxreorg-depth");
    }
    ...
    //
    // This is a sync-level refusal, not evidence the peer is malicious. The
    // legacy client declines the same block with a plain error() and no DoS
    // score at all; scoring it as BLOCK_INVALID_HEADER cost the peer 100
    // points, an immediate ban. On a network where the legacy nodes are the
    // majority, that meant banning honest peers for offering a fork our
    // checkpoint happens to sit in front of. BLOCK_HEADER_SYNC scores 1, which
    // is far closer to what the rest of the network does. Matches upstream
    // blackcoin-more 1aa978dab6.
    ...
    // Retain the parent-time rule used by the proven historical replay. The
    // stricter legacy-source comparison needs a full-history and regtest gate
    // before it can safely replace this rule.
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // BIP34 remains parked on mainnet. Global coinbase-height enforcement was
    // removed by the early block-by-block reconciliation after it rejected
    // canonical history; do not restore it without proven exceptions.
    ...
    // Cap the witness-inclusive serialized block size at MAX_BLOCK_SERIALIZED_SIZE. Pre-SegWit the
    // weight limit (4 x MAX_BLOCK_SERIALIZED_SIZE) already bounded the serialized size; once SegWit
    // activates the witness discount would otherwise let a within-weight block exceed it, which the
    // P2P message and block-file limits (also MAX_BLOCK_SERIALIZED_SIZE) can neither relay nor reload.
    if (::GetSerializeSize(TX_WITH_WITNESS(block)) > MAX_BLOCK_SERIALIZED_SIZE) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-length", "size limits failed");
    }
...
// Headers a peer may run ahead of the validated tip before elapsed time is taken
// into account. An honest peer announces at most MAX_BLOCKS_TO_ANNOUNCE (8)
// unsolicited headers, so this leaves room for two such bursts while a caught-up
// node is at the tip. Anything longer has to be earned by elapsed time below.
static constexpr int64_t HEADER_LEAD_BASE{16};
// Extra headers allowed per nTargetSpacing of elapsed time since the validated
// tip. Generous against honest staking jitter while still bounding a peer that
// mints stake headers for free.
static constexpr int64_t HEADER_LEAD_RATE_FACTOR{4};
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked, bool fOldClient)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        bool proof_type_unproven{false};
        if (!CheckBlockHeader(block, state, GetConsensus(), chainstate, !fProofOfStake, fOldClient, &proof_type_unproven))
        {
            // Only the unproven work/stake guess may be retried. Flipping on any failure
            // instead sent a valid proof-of-work block on to be measured against the
            // proof-of-stake target, which reported "bad-diffbits, incorrect
            // proof-of-stake" and scored the sender 100 for what was really a few minutes
            // of clock drift. The state check is kept alongside the flag so a verdict that
            // is already recorded can never be overridden by a retry.
            if (fOldClient && proof_type_unproven && !state.IsInvalid())
        ...
        // Anti-DoS policy, not a consensus rule. A proof-of-stake header proves
        // nothing until its block arrives, so a peer owning no stake can mint
        // them for free and run the index far past the validated tip. An honest
        // chain still needs real time to grow, about one block per
        // nTargetSpacing, so headers reaching further ahead than elapsed time
        // could have produced are refused. Nothing is marked invalid and the
        // peer may send them again once time has passed. The allowance scales
        // with how far behind the validated tip is, so initial sync and
        // catching up after downtime are unaffected.
        if (const CBlockIndex* chain_tip = chainstate.m_chain.Tip()) {
            const int height{pindexPrev->nHeight + 1};
            const int64_t spacing{std::max<int64_t>(1, GetConsensus().nTargetSpacing)};
            const int64_t elapsed{std::max<int64_t>(0, TicksSinceEpoch<std::chrono::seconds>(m_options.adjusted_time_callback()) - chain_tip->GetBlockTime())};
            const int64_t allowance{HEADER_LEAD_BASE + (elapsed / spacing) * HEADER_LEAD_RATE_FACTOR};
            if (height > chain_tip->nHeight + allowance) {
                LogPrint(BCLog::VALIDATION, "%s: header %s at height %d runs %d ahead of validated tip %d, allowance %d\n",
                         __func__, hash.ToString(), height, height - chain_tip->nHeight, chain_tip->nHeight, allowance);
                return state.Invalid(BlockValidationResult::BLOCK_HEADER_LOW_WORK, "header-too-far-ahead");
            }
        }
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // Prove the stake kernel before this block is written to disk, the way the
    // legacy client does (it calls CheckProofOfStake in AcceptBlock, ahead of
    // WriteToDisk).  Headers-first sync moved that proof to ConnectBlock, so
    // until here nothing has asked whether the coinstake's prevout exists, is
    // mature, or meets target -- CheckBlock only checks that a coinstake is
    // present and correctly timestamped.  A peer can therefore mint unlimited
    // sibling blocks at tip+1, each with a different bogus coinstake and so a
    // different hash, each carrying more work than the tip, and have every one
    // of them stored.  The header lead bound does not help: it limits how far
    // ahead a chain runs, not how wide it is.
    //
    // The check is deliberately restricted to a block that extends the active
    // tip while we are not in initial block download, because that is exactly
    // when CoinsTip() is the view this block must be judged against.  A side
    // branch needs the UTXO set as of its fork point, which only ConnectBlock
    // has, so those keep today's behaviour.  Nothing here changes what the
    // network accepts: a block rejected below is one ConnectBlock would have
    // rejected anyway, minus the disk write.  It is not marked permanently
    // invalid, so a peer may offer it again.
    if (block.IsProofOfStake() && !IsInitialBlockDownload() && ActiveTip() == pindex->pprev &&
        params.GetConsensus().IsProtocolV3(block.GetBlockTime())) {
        if (!CheckProofOfStake(pindex->pprev, *block.vtx[1], block.nBits, state,
                               ActiveChainstate().CoinsTip(), block.vtx[1]->nTime)) {
            LogPrint(BCLog::VALIDATION, "%s: stake kernel unproven for tip-extending block %s, not storing: %s\n",
                     __func__, block.GetHash().ToString(), state.ToString());
            return false;
        }
    }
...
{
    // DER only, matching the 2.x majority.
    //
    // vchBlockSig is serialized after vtx and is not covered by the block hash,
    // so any relaying peer can negate S without changing the block's identity.
    // Legacy 2.x accepts a high-S block signature -- IsCanonicalBlockSignature
    // there tests IsDERSignature alone -- and repairs the encoding rather than
    // punishing the sender. Rejecting the low-S form here scored the peer 100
    // and banned it, so a single malleated relay evicted honest majority nodes
    // that had done nothing wrong, and the same block still arrived intact from
    // anyone else.
    return IsCanonicalBlockSignature(pblock, false);
}
