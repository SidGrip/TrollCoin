...
static std::map<std::string, unsigned int> mapFlagNames = {
    ...
    // TrollCoin keeps SCRIPT_VERIFY_DERKEY in MANDATORY_SCRIPT_VERIFY_FLAGS (and therefore in
    // STANDARD_SCRIPT_VERIFY_FLAGS), unlike upstream where the flag is unused, so it has to be
    // named here or CheckMapFlagNames() reports it as missing.
    {std::string("DERKEY"), (unsigned int)SCRIPT_VERIFY_DERKEY},
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    // re-encoded in TrollCoin's wire format: nTime (here 0) is serialized between nVersion and vin,
    // so the four zero bytes after the version are part of the header, not of the input count. Only
    // CheckTransaction() is exercised below, so no signature depends on the nTime value.
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    // IsStandardTx()'s witnessEnabled parameter defaults to false, but the node always passes the
    // real deployment state (MemPoolAccept::PreChecks). Pass true so the segwit outputs below are
    // checked for dust instead of being rejected as "scriptpubkey" before the dust branch is
    // reached, which would silently drop all segwit dust coverage.
    ...
    // Check dust with default relay fee. TrollCoin's DUST_RELAY_TX_FEE is 10000 sat/kvB (matching
    // the 2.x fee schedule), not Bitcoin's 3000, so every dust threshold below is 10x the upstream
    // number: 10000/1000 divides exactly, so GetFee(n) == 10*n with no rounding term.
    ...
    // TrollCoin carries 220 data bytes (MAX_OP_RETURN_RELAY 223), not Bitcoin's 80 (83), so the
    // payload is sized off the constant rather than spelled out as a hex literal. Both 220 and 221
    // are pushed with OP_PUSHDATA1, hence the script is exactly OP_RETURN + 2 + payload bytes.
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(MAX_OP_RETURN_RELAY - 3, 0);
    BOOST_CHECK_EQUAL(MAX_OP_RETURN_RELAY, t.vout[0].scriptPubKey.size());
    CheckIsStandard(t);

    // MAX_OP_RETURN_RELAY+1-byte TxoutType::NULL_DATA (non-standard)
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(MAX_OP_RETURN_RELAY - 2, 0);
    ...
    // The TrollCoin tx header is 4 bytes longer than Bitcoin's because nTime is serialized right
    // after nVersion, so the output is shrunk by the same 4 bytes to keep the totals exact.
    ...
    // Every threshold below is GetDustThreshold() worked out by hand for this chain:
    // (8 nValue + 1 script-length varint + script bytes) plus the spending input (148 bytes for a
    // bare input, 67 for a witness program), times DUST_RELAY_TX_FEE/1000 == 10 sat per byte.
    // They are therefore 10x the upstream (3000 sat/kvB) numbers, not arbitrary edits.

    // Check compressed P2PK outputs dust threshold (must have leading 02 or 03)
    t.vout[0].scriptPubKey = CScript() << std::vector<unsigned char>(33, 0x02) << OP_CHECKSIG;
    t.vout[0].nValue = 1920; // (8 + 1 + 35 + 148) * 10
    CheckIsStandard(t);
    t.vout[0].nValue = 1919;
    CheckIsNotStandard(t, "dust");
    ...
    t.vout[0].nValue = 2240; // (8 + 1 + 67 + 148) * 10
    CheckIsStandard(t);
    t.vout[0].nValue = 2239;
    CheckIsNotStandard(t, "dust");
    ...
    t.vout[0].nValue = 1100; // (8 + 1 + 34 + 67) * 10
    CheckIsStandard(t);
    t.vout[0].nValue = 1099;
    CheckIsNotStandard(t, "dust");
    ...
    for (int op = OP_1; op <= OP_16; op += 1) {
        ...
        t.vout[0].nValue = 800; // (8 + 1 + 4 + 67) * 10
        CheckIsStandard(t);
