...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            result[outpoint] = Coin{tx.vout[0], height, CTransaction{tx}.IsCoinBase(), CTransaction{tx}.IsCoinStake(), static_cast<int>(tx.nTime)};
...
{
    const auto check_roundtrip = [](const Coin& coin, const std::string& expected) {
        DataStream serialized;
        serialized << coin;
        BOOST_CHECK_EQUAL(HexStr(serialized), expected);
    };
    ...
    check_roundtrip(cc1, "b0e57800835800816115944e077fe7c803cfa57f29b36bf87c1d35");
    ...
    BOOST_CHECK_EQUAL(cc2.nTime, 0U);
    ...
    check_roundtrip(cc2, "9cc06d00bbd123008c988f1a4a4de2161e0f50aac7f17e7f9555caa4");

    // Nonzero nTime and the coinstake bit are persisted with the UTXO.
    DataStream ss_coinstake{ParseHex("802ab9ee99150006")};
    Coin cc_coinstake;
    ss_coinstake >> cc_coinstake;
    BOOST_CHECK_EQUAL(cc_coinstake.fCoinBase, false);
    BOOST_CHECK_EQUAL(cc_coinstake.fCoinStake, true);
    BOOST_CHECK_EQUAL(cc_coinstake.nHeight, 42U);
    BOOST_CHECK_EQUAL(cc_coinstake.nTime, 123456789U);
    BOOST_CHECK_EQUAL(cc_coinstake.out.nValue, 0);
    BOOST_CHECK_EQUAL(cc_coinstake.out.scriptPubKey.size(), 0U);
    check_roundtrip(cc_coinstake, "802ab9ee99150006");
    ...
    BOOST_CHECK_EQUAL(cc3.nTime, 0U);
    BOOST_CHECK_EQUAL(cc3.out.nValue, 0);
    BOOST_CHECK_EQUAL(cc3.out.scriptPubKey.size(), 0U);
    check_roundtrip(cc3, "00000006");
