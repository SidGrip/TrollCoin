...
void static RandomTransaction(CMutableTransaction& tx, bool fSingle)
{
    ...
    // TrollCoin commits to nTime in the signature hash, so randomise it here
    // rather than inheriting one wall-clock value for every iteration.
...
//
// data/sighash.json holds the upstream Bitcoin corpus with a deterministic
// 4-byte nTime spliced in after each raw transaction's nVersion, and every
// expected hash recomputed: TrollCoin transactions carry nTime on the wire and
// CTransactionSignatureSerializer commits to it, so the upstream transactions
// no longer deserialize and the upstream hashes no longer apply.
