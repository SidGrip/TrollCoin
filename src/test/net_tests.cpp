...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Then send a too-large message. Derived from the node's own cap rather than a
        // literal: TrollCoin raises MAX_PROTOCOL_MESSAGE_LENGTH to 8 MB (7 MB blocks), so
        // upstream's 4,005,000 is a legal message here and would not be rejected.
        auto msg_data_3 = g_insecure_rand_ctx.randbytes<uint8_t>(std::min<size_t>(MAX_SIZE, MAX_PROTOCOL_MESSAGE_LENGTH) + 5000);
