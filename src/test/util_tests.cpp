...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    // The only ceiling ParseMoney() applies is MoneyRange(), and TrollCoin's
    // MAX_MONEY is 900000000 * COIN rather than Bitcoin's 21000000 * COIN, so
    // the boundary sits here. Pin both sides of it.
    BOOST_CHECK_EQUAL(ParseMoney("900000000.00").value(), MAX_MONEY);

    // Parsing amount that cannot be represented should fail
    BOOST_CHECK(!ParseMoney("900000000.01"));
    BOOST_CHECK(!ParseMoney("900000001.00"));
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    // Capitalize() only uppercases the first character, so the branded name
    // round-trips as "Trollcoin" here; this is not a statement about how the
    // coin is spelled elsewhere.
    BOOST_CHECK_EQUAL(Capitalize("trollcoin"), "Trollcoin");
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // derived address from this private key: TdWEwRJzHirPK7MejymKvmbqGHdb41rKE3
        // (TrollCoin PUBKEY_ADDRESS version byte is 66, not Bitcoin's 0)
    ...
    // Signing is RFC6979-deterministic, so this vector is fixed once the magic
    // is. It differs from upstream Bitcoin's because MESSAGE_MAGIC here is
    // "TrollCoin Signed Message:\n", which changes the hash that gets signed.
    const std::string expected_signature =
        "H6cD6OPp1RjIEI+mXB23QTo/gJlbU7pw6ztK42WoK1siHuKoSDFVqt9AdRXX0qMBzGpPbzvaTtfQ/YYvaFNDbh8=";
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    // The P2PKH addresses below use TrollCoin's PUBKEY_ADDRESS version byte 66;
    // the upstream "1..." (version 0) vectors would stop at ERR_INVALID_ADDRESS
    // here. The P2SH vector above needs no change - SCRIPT_ADDRESS is still 5.
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "TdWEwRJzHirPK7MejymKvmbqGHdb41rKE3",
    ...
    // Same handcrafted signature blob as upstream, but the key it recovers
    // depends on the message hash, so the address it belongs to is chain
    // specific. Upstream's recovered hash160 happened to start with a zero byte
    // and rendered as "11canuhp..."; that leading-zero base58 case cannot occur
    // under version byte 66 and is covered by base58_tests instead.
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "Ts22e8Vo4d5yL68AJYhPzwEyf4mFcEQRH9",
