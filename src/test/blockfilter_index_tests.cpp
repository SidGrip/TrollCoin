    ...
    const CScript& scriptPubKey)
{
    ...
    }
    {
        ...
        // The block time was moved back to the parent's, so restamp the coinbase
        // to match; CheckBlock rejects a block older than a transaction it carries.
        tx_coinbase.nTime = block.nTime;
        block.vtx.at(0) = MakeTransactionRef(std::move(tx_coinbase));
        block.hashMerkleRoot = BlockMerkleRoot(block);
    }
    // The header fields above were mutated in place, so the scrypt hash the
    // template cached is stale and would be mined against instead of the real one.
