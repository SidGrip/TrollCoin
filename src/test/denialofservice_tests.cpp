...
BOOST_AUTO_TEST_CASE(stale_tip_peer_management)
{
    ...
    // The stale-tip check only runs when GetTime() has advanced past m_stale_tip_check_time,
    // which the first CheckForStaleTipAndEvictPeers() below sets to now + STALE_CHECK_INTERVAL
    // (a fixed 10 minutes). Advancing only 3*nTargetSpacing works on Bitcoin because its
    // spacing is 600s and so exceeds that interval; TrollCoin's 64s spacing gives 193s, which
    // never clears the gate. Advance past the 10-minute interval AND the staleness threshold.
    const auto time_later{time_init + 10min
                          + 3 * std::chrono::seconds{m_node.chainman->GetConsensus().nTargetSpacing} + 1s};
