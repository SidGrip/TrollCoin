...
BOOST_AUTO_TEST_CASE(siphash)
{
    ...
    // CMutableTransaction's default constructor seeds nTime from GetAdjustedTimeSeconds(),
    // so the serialization -- and therefore this hash -- changed on every run and the check
    // could never pass. Pin both fields: the vector below is for nVersion=2, nTime=0, no
    // inputs or outputs and nLockTime=0, and was computed independently of the node.
    tx.nVersion = 2;
    tx.nTime = 0;
    ss << TX_WITH_WITNESS(tx);
    BOOST_CHECK_EQUAL(SipHashUint256(1, 2, ss.GetHash()), 0x47085ccf473fbbfbULL);
