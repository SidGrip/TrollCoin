...
// A fee amount that is below TrollCoin's flat 10sat/vB floor (DEFAULT_MIN_RELAY_TX_FEE) for every
// transaction created within these unit tests, so a parent paying it can only reach the mempool if
// a child pays for it through the package feerate. Upstream this value sat in the band between min
// relay (1sat/vB) and a mocked mempool minimum (5sat/vB); this chain has no rolling mempool minimum
// (CTxMemPool::GetMinFee() is compiled out, src/txmempool.cpp:1121-1154), so that band does not
// exist and there is a single floor to be under.
...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    // TrollCoin's proof-of-work schedule does not pay Bitcoin's flat 50 BTC: block 1 mints the
    // 500,000,000 TROLL v1 swap allocation and blocks 2.. pay 125 TROLL (GetProofOfWorkSubsidy(),
    // src/validation.cpp). Derive the fee from the coinbase actually being spent so the 1 COIN fee
    // the checks below expect holds on any schedule. Amount fields are fixed-width, so vsize is
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    // See package_validation_tests: TrollCoin coinbases are not 50 COIN, so derive each generation's
    // 1 COIN fee from the coinbase actually being spent.
    const CAmount coinbase_value{m_coinbase_txns[0]->vout[0].nValue};
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/parent_locking_script,
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mineBlocks(5);
    // No MockMempoolMinFee() here: this chain has no rolling mempool minimum feerate (GetMinFee()
    // and trackPackageRemoved() are compiled out, src/txmempool.cpp:1121-1154), so the helper cannot
    // raise the pool floor above m_min_relay_feerate and would only assert on the way through.
    LOCK(cs_main);
    ...
    // See package_validation_tests: TrollCoin coinbases are not 50 COIN, so derive the amounts from
    // the coinbase actually being spent.
    const CAmount coinbase_value_0{m_coinbase_txns[0]->vout[0].nValue};
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/scriptPubKey,
                                                    /*output_amount=*/coinbase_value_0 - COIN, /*submit=*/false);
    ...
    mtx_child1.vout[0].nValue = coinbase_value_0 - 2 * COIN;
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/ptx_child2, /*input_vout=*/0,
                                                        ...
                                                        /*output_destination=*/grandchild_locking_script,
                                                        /*output_amount=*/coinbase_value_0 - 3 * COIN, /*submit=*/false);
    ...
    const CAmount parent1_value{coinbase_value_1 - COIN};
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/parent1_value, /*submit=*/true);
    ...
    const CAmount coinbase_value_2{m_coinbase_txns[2]->vout[0].nValue};
    auto mtx_grandparent2 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                          ...
                                                          /*output_amount=*/coinbase_value_2 - COIN, /*submit=*/true);
    ...
    const CAmount parent2_value{coinbase_value_2 - 2 * COIN};
    mtx_parent2_v1.vout[0].nValue = parent2_value;
    ...
    const CAmount coinbase_value_3{m_coinbase_txns[3]->vout[0].nValue};
    const CAmount parent3_value{coinbase_value_3 - low_fee_amt};
    auto mtx_parent3 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[3], /*input_vout=*/0,
                                                     /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                     /*output_destination=*/acs_spk,
                                                     /*output_amount=*/parent3_value, /*submit=*/false);
    CTransactionRef ptx_parent3 = MakeTransactionRef(mtx_parent3);
    package_mixed.push_back(ptx_parent3);
    // Upstream asserted two things here: parent3 is under the mocked mempool minimum but still over
    // min relay, so only a package could carry it. This chain has a single floor -- GetMinFee() is
    // compiled out of CTxMemPool (src/txmempool.cpp:1121-1154) and MemPoolAccept::CheckFeeRate falls
    // back on m_min_relay_feerate -- so the two halves collapse into one: parent3 must simply be
    // under that floor, which is what makes it fail on its own and need the package feerate.
    ...
    // parent3 already paid low_fee_amt; the child pays the rest of the 1 COIN package fee, so the
    // package feerate below is exactly 1 COIN over parent3 + child regardless of the subsidy.
    const CAmount mixed_child_fee{1 * COIN - low_fee_amt};
    mtx_mixed_child.vout.emplace_back(parent1_value + parent2_value + parent3_value - mixed_child_fee, mixed_child_spk);
    ...
    // child should be accepted
    {
        ...
        const CFeeRate expected_feerate(low_fee_amt + mixed_child_fee, GetVirtualTransactionSize(*ptx_parent3) + GetVirtualTransactionSize(*ptx_mixed_child));
...
{
    mineBlocks(5);
    // No MockMempoolMinFee() here: see package_witness_swap_tests. Without a rolling mempool
    // minimum the only floor a package has to clear is m_min_relay_feerate, i.e. a flat 10sat/vB.
    LOCK(::cs_main);
    ...
    // See package_validation_tests: TrollCoin coinbases are not 50 COIN, so take the value from the
    // coinbase actually being spent.
    const CAmount coinbase_value{m_coinbase_txns[0]->vout[0].nValue};
    ...
    m_node.mempool->PrioritiseTransaction(tx_parent->GetHash(), child_value - coinbase_value);
    {
        ...
        // Upstream this fails per-transaction (PCKG_TX), because there the min relay feerate is also
        // enforced on each transaction inside MemPoolAccept::PreChecks. Here PreChecks only applies
        // the fee gate when it is not doing package feerates (src/validation.cpp), so the
        // de-prioritised parent survives PreChecks and the whole package is what gets rejected.
        ...
        BOOST_CHECK_EQUAL(submit_cpfp_deprio.m_state.GetRejectReason(), "package-fee-too-low");
        ...
        // The parent's individual result comes from the GetMinFee() gate, which MemPoolAccept
        // applies ahead of the min relay gate (src/validation.cpp, MemPoolAccept::CheckFeeRate), so
        // the reason is "min fee not met" rather than upstream's "min relay fee not met".
    ...
    // The floor is a flat 10sat/vB, but this package just pays 800 satoshis total. Unlike upstream
    // the child cannot pay for itself at this floor either; the package-level rejection is what is
    ...
    auto mtx_parent_cheap = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                          /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                          /*output_destination=*/parent_spk,
                                                          /*output_amount=*/coinbase_value_1 - parent_fee, /*submit=*/false);
    CTransactionRef tx_parent_cheap = MakeTransactionRef(mtx_parent_cheap);
    package_still_too_low.push_back(tx_parent_cheap);
    // As in package_witness_swap_tests: there is only one floor on this chain, so the upstream pair
    // of "under the mempool minimum" and "over min relay" collapses into the parent simply being
    // under m_min_relay_feerate, which is what makes it fail on its own.
    ...
    auto mtx_child_cheap = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent_cheap, /*input_vout=*/0,
                                                         /*input_height=*/101, /*input_signing_key=*/child_key,
                                                         /*output_destination=*/child_spk,
                                                         /*output_amount=*/coinbase_value_1 - parent_fee - child_fee, /*submit=*/false);
    CTransactionRef tx_child_cheap = MakeTransactionRef(mtx_child_cheap);
    package_still_too_low.push_back(tx_child_cheap);
    // ...and the two of them together are still under it, which is what package-fee-too-low means.
    BOOST_CHECK(m_node.mempool->m_min_relay_feerate.GetFee(GetVirtualTransactionSize(*tx_parent_cheap) + GetVirtualTransactionSize(*tx_child_cheap)) > parent_fee + child_fee);
    ...
    auto mtx_parent_rich = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                         /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                         /*output_destination=*/parent_spk,
                                                         /*output_amount=*/coinbase_value_2 - high_parent_fee, /*submit=*/false);
    ...
    // Parent pays 1 BTC and child pays none. The parent should be accepted without the child.
    {
        ...
        // MemPoolAccept::CheckFeeRate applies the GetMinFee() gate ahead of the min relay gate, so a
        // zero-fee transaction is turned away as "min fee not met" on this chain.
        BOOST_CHECK(it_child->second.m_state.GetRejectReason() == "min fee not met");
