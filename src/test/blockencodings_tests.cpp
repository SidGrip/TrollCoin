...
// One fixed past timestamp for every synthetic transaction and block. CheckBlock
// rejects a block older than a transaction it carries, and CMutableTransaction
// otherwise stamps the current time, which would make mining non-deterministic.
static constexpr uint32_t TEST_BLOCK_TIME{1600000000};
...
static CBlock BuildBlockTestCase() {
    ...
    // TrollCoin rejects nVersion > 1, and above 6 GetHash() switches to sha256d
    // while validation mines and checks the scrypt GetPoWHash().
    block.nVersion = 1;
    block.nTime = TEST_BLOCK_TIME;
    ...
    while (!CheckProofOfWork(block.GetHash(), block.nBits, Params().GetConsensus())) {
        ++block.nNonce;
        block.InvalidateCache(); // nNonce changed -> cached scrypt hash is stale
    }
    return block;
}
...
BOOST_AUTO_TEST_CASE(EmptyBlockRoundTripTest)
{
    ...
    coinbase.nTime = TEST_BLOCK_TIME;
