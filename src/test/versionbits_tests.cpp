...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    if (nTime == 0) {
        ...
        // Now mine more blocks at the start time. TrollCoin's GetMedianTimePast()
        // returns the block's own timestamp once ProtocolV2 is in force
        // (CBlockIndex::GetMedianTimePast() in chain.h; mainnet nProtocolV2Time == 0),
        // so there is no 11-block median lag: CBV keeps the bit clear only while the
        // period-boundary block still predates nStartTime.
        nTime = nStartTime;
        for (uint32_t i = params.nMinerConfirmationWindow - 4; i < params.nMinerConfirmationWindow; i++) {
            lastBlock = firstChain.Mine(params.nMinerConfirmationWindow + i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
        }
        // The period-boundary block itself carries nStartTime, so it transitions
        // straight to STARTED here rather than at the following period.
        lastBlock = firstChain.Mine(params.nMinerConfirmationWindow * 2, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit)) != 0);
        // It stays STARTED as we advance into the next period,
    }
