    ...
    : TestingSetup{chain_type, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    constexpr int test_chain_height{100};
    this->mineBlocks(test_chain_height);
    {
        LOCK(::cs_main);
        Assert(m_node.chainman->ActiveChain().Height() == test_chain_height);
    }
    Assert(m_coinbase_txns.size() == static_cast<size_t>(test_chain_height));
}
...
void TestChain100Setup::mineBlocks(int num_blocks)
{
    ...
    for (int i = 0; i < num_blocks; i++) {
        ...
        Assert(WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip()->GetBlockHash() == b.GetHash()));
    ...
    Chainstate& chainstate)
{
    ...
    while (!CheckProofOfWork(block.GetHash(), block.nBits, m_node.chainman->GetConsensus())) { ++block.nNonce; block.InvalidateCache(); }
...
// Inert on this chain: there is no rolling minimum feerate to raise. CTxMemPool::GetMinFee() and
// trackPackageRemoved() are compiled out and TrimToSize() no longer tracks the removed feerate
// (src/txmempool.cpp), so after the TrimToSize(0) below the pool floor is still exactly
// m_min_relay_feerate. Kept for signature compatibility with upstream; callers that need a
// "below the mempool minimum but above min relay" band have no such band to work with here.
