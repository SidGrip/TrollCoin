...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
...

    {
        ...
        // Upstream built a 210000-entry synthetic chain index here to straddle Bitcoin's
        // halving and assert CreateNewBlock() still worked either side of it. TrollCoin has no
        // such boundary: GetProofOfWorkSubsidy steps at 777776/1555553/2333330, so 209999 and
        // 210000 both sit inside the same 125-TROLL band and the two calls compared identical
        // subsidies. Removing it is worthwhile on its own terms, and it does let the suite
        // finish where it previously hit a 40-minute timeout -- but it is NOT the whole cost:
        // the suite still takes minutes afterwards, so most of the remaining time is elsewhere
        // (the 110-block scrypt bootstrap and the package/priority cases are the candidates,
        // unmeasured). Do not read this deletion as a runtime fix. The subsidy step itself is
        // covered directly and far more precisely by subsidy_schedule below; what remains here
        // is the assembler behaviour this block was always really testing.
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto& bi : BLOCKINFO) {
        ...
        {
            LOCK(cs_main);
            // TrollCoin blocks are version 1: CheckBlockHeader rejects nVersion > 1 outright,
            // because the legacy 2.x client never bumped past 1 and must keep accepting what
            // this chain mines. VERSIONBITS_TOP_BITS is also > 6, which would send GetHash()
            // down the sha256d path while validation checks the scrypt GetPoWHash(), so the
            // nonce search below would have solved the wrong hash.
            pblock->nVersion = 1;
            pblock->nTime = m_node.chainman->ActiveChain().Tip()->GetMedianTimePast()+1;
            CMutableTransaction txCoinbase(*pblock->vtx[0]);
            txCoinbase.nVersion = 1;
            // TrollCoin requires block time >= every transaction's nTime (validation.cpp,
            // "bad-tx-time"). CreateNewBlock stamped this coinbase from the adjusted clock,
            // but pblock->nTime above was forced back to MedianTimePast+1, so the coinbase
            // was left dated after its own block and all 110 blocks were refused.
            ...
            // The template is built once and reused for all 110 blocks, so its nBits is whatever
            // the tip required at that moment. ContextualCheckBlockHeader compares nBits against
            // what consensus requires at this height, so it has to be recomputed per block --
            // and on regtest that lands on the mindiff branch, which is what keeps the search
            // below cheap.
            pblock->nBits = GetNextTargetRequired(m_node.chainman->ActiveChain().Tip(),
                                                  m_node.chainman->GetConsensus(), /*fProofOfStake=*/false);
            // BLOCKINFO's nonces were mined against Bitcoin's header serialization and sha256d
            // block identity. A block here is identified by scrypt(1024,1,1) over a header that
            // does not match Bitcoin's, so the upstream nonces solve nothing and this table was
            // regenerated for TrollCoin. bi.extranonce still distinguishes the coinbases.
            pblock->nNonce = bi.nonce;
            // Every field above changed, and GetPoWHash() memoizes the scrypt result on the
            // header, so the cache must be dropped before testing -- otherwise the previous
            // block's hash is checked instead of this one's.
            pblock->InvalidateCache();
            if (!CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, m_node.chainman->GetConsensus())) {
                // The stored nonce does not solve this header. Search for one and print it, so a
                // single regeneration run emits a whole replacement table. This costs 2^32/target
                // scrypt evaluations at MAINNET difficulty -- TestingSetup defaults to
                // ChainType::MAIN -- which is exactly why upstream pre-mines these offline rather
                // than searching at runtime. Do not "simplify" this into an unconditional search:
                // that turns a one-time offline cost into a multi-hour cost on every run.
                pblock->nNonce = 0;
                pblock->InvalidateCache();
                while (!CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, m_node.chainman->GetConsensus())) {
                    ++pblock->nNonce;
                    pblock->InvalidateCache();
                }
                fprintf(stderr, "REGEN {%u, %u},\n", (unsigned)bi.extranonce, pblock->nNonce);
            }
        }
...
// Replaces the 210000-entry synthetic-chain "subsidy changing" section that CreateNewBlock_validity
// used to carry. That section straddled Bitcoin's halving, which does not exist on this chain, and
// only ever asserted that CreateNewBlock() did not throw -- it never compared a subsidy. This
// checks the schedule itself at every boundary, costs microseconds, and is independent of the
// block assembler.
//
// Note the off-by-one that the schedule is written around: GetProofOfWorkSubsidy computes from
// h = nHeight - 1, because TrollCoin pays block N the value for the tip height it was built on.
// So the step that the source spells `h <= 777776` falls between block heights 777777 and 777778.
BOOST_AUTO_TEST_CASE(subsidy_schedule)
{
    // Block 1 mints the v1 swap allocation via the h <= 0 branch.
    BOOST_CHECK_EQUAL(GetProofOfWorkSubsidy(1), 500000000 * COIN);
    BOOST_CHECK_EQUAL(GetProofOfWorkSubsidy(2), 125 * COIN);

    // Each pair straddles one step: last height paying the old value, first paying the new one.
    const std::vector<std::tuple<int, CAmount, CAmount>> steps{
        {777776,  125 * COIN,  60 * COIN},
        {1555553,  60 * COIN,  34 * COIN},
        {2333330,  34 * COIN,  20 * COIN},
        {3111107,  20 * COIN,  17 * COIN},
        {3888884,  17 * COIN,  15 * COIN},
        {4666662,  15 * COIN,  12 * COIN},
        {5444438,  12 * COIN,  10 * COIN},
        {6222215,  10 * COIN,   8 * COIN},
        {6999992,   8 * COIN,   7 * COIN},
    };
    for (const auto& [h, before, after] : steps) {
        BOOST_CHECK_EQUAL(GetProofOfWorkSubsidy(h + 1), before);   // h == boundary
        BOOST_CHECK_EQUAL(GetProofOfWorkSubsidy(h + 2), after);    // h == boundary + 1
    }

    // Proof-of-stake pays a flat amount regardless of height.
    BOOST_CHECK_EQUAL(GetProofOfStakeSubsidy(), 7 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(1, Params().GetConsensus(), /*fProofOfStake=*/true), 7 * COIN);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(8000000, Params().GetConsensus(), /*fProofOfStake=*/true), 7 * COIN);

    // GetBlockSubsidy dispatches to the proof-of-work schedule when not staking.
    BOOST_CHECK_EQUAL(GetBlockSubsidy(2, Params().GetConsensus(), /*fProofOfStake=*/false), 125 * COIN);
}
