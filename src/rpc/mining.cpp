...
static bool GenerateBlock(ChainstateManager& chainman, CBlock& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    block.InvalidateCache(); // merkle root changed -> any hash cached during template build (TestBlockValidity) is stale
    ...
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !CheckProofOfWork(block.GetPoWHash(), block.nBits, chainman.GetConsensus()) && !ShutdownRequested()) {
        ++block.nNonce;
        block.InvalidateCache(); // nNonce changed -> the cached scrypt hash is stale
        --max_tries;
    }
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated trollcoin to."},
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated trollcoin to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
         ...
         RPCExamples{
            ...
            + "If you are using the " PACKAGE_NAME " wallet, you can get a new address to send the newly generated trollcoin to with:\n"
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated trollcoin to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Read the live tip, not the cached pindexPrev (still stale/null on the first call after a new
    // block), so the segwit-rule check and the "rules" list match the template actually built below.
    ...
    // Buried (height) soft forks — advertise each only once active for the block being built,
    // so a template before the flag-day height lists none of them and after it lists all.
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CSV)) aRules.push_back("csv");
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_DERSIG)) aRules.push_back("bip66");
