    ...
    return
         {
             ...
             {RPCResult::Type::STR, "address", /*optional=*/true, "The TrollCoin address (only if a well-defined address exists)"},
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed TrollCoin transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed TrollCoin transactions into one transaction.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (unsigned int i = 0; i < txs.size(); ++i) {
        ...
        // A joined TrollCoin transaction must not predate any source PSBT.
        latest_time = std::max(latest_time, psbtx.tx->nTime);
    ...
    merged_psbt.tx->nTime = latest_time;
    ...
    shuffled_psbt.tx->nTime = merged_psbt.tx->nTime;
