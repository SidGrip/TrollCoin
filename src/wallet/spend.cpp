...
//! A borrowed view of a SigningProvider, for handing one to MultiSigningProvider without
//! copying it. MultiSigningProvider::AddProvider takes ownership, so handing it the external
//! provider directly would copy all five maps at every merge site, and both sites are in a
//! loop: once per candidate coin in AvailableCoins while holding cs_wallet, and up to twice
//! per input in GetDescriptor -- CalculateMaximumSignedTxSize reaches it once per txout in
//! the is_segwit pre-scan (short-circuiting at the first segwit hit) and again per input via
//! GetSignedTxinWeight. Those copies would scale with data that cannot change during the
//! walk, which is why this exists; the merge as written copies nothing.
//!
//! Lifetime: the referent must outlive the view. Both current uses borrow a CCoinControl
//! owned by the caller, and in each case the MultiSigningProvider holding the view is a
//! local that dies first -- AvailableCoins' per-output `provider`, and GetDescriptor's
//! `providers`. A new use must satisfy the same property; this class cannot enforce it.
class BorrowedSigningProvider final : public SigningProvider
{
    const SigningProvider& m_ref;
public:
    explicit BorrowedSigningProvider(const SigningProvider& ref) : m_ref(ref) {}
    bool GetCScript(const CScriptID& id, CScript& out) const override { return m_ref.GetCScript(id, out); }
    bool HaveCScript(const CScriptID& id) const override { return m_ref.HaveCScript(id); }
    bool GetPubKey(const CKeyID& id, CPubKey& out) const override { return m_ref.GetPubKey(id, out); }
    bool GetKey(const CKeyID& id, CKey& out) const override { return m_ref.GetKey(id, out); }
    bool HaveKey(const CKeyID& id) const override { return m_ref.HaveKey(id); }
    bool GetKeyOrigin(const CKeyID& id, KeyOriginInfo& out) const override { return m_ref.GetKeyOrigin(id, out); }
    bool GetTaprootSpendData(const XOnlyPubKey& k, TaprootSpendData& out) const override { return m_ref.GetTaprootSpendData(k, out); }
    bool GetTaprootBuilder(const XOnlyPubKey& k, TaprootBuilder& out) const override { return m_ref.GetTaprootBuilder(k, out); }
};

//! Does this coin control carry externally-supplied solving material?
//!
//! One predicate rather than an inline member list, because the list is the part that
//! rots: the first version tested pubkeys/scripts/origins/tr_trees and silently ignored
//! keys, quietly deciding that keys alone "do not count" -- a policy nobody chose. Adding
//! a field to FlatSigningProvider now means updating one named function.
//!
//! Not replaced by an unconditional merge, which would match GetDescriptor's shape more
//! closely: merging always turns a null wallet provider into a non-null one that solves
//! nothing, and CalculateMaximumSignedInputSize short-circuits to -1 on null but runs
//! InferDescriptor otherwise. Whether that inference stays unsolvable for every script is
//! not something to assume on a path every wallet caller takes, so the guard keeps the
//! no-op provable instead.
static bool HasExternalSolvingData(const FlatSigningProvider& p)
{
    return !p.pubkeys.empty() || !p.scripts.empty() || !p.origins.empty()
        || !p.tr_trees.empty() || !p.keys.empty();
}
                                                 ...
                                                 const CScript script_pubkey)
{
    ...
    if (coin_control) {
        // Borrowed, not copied: CalculateMaximumSignedTxSize reaches this up to twice per
        // input -- the is_segwit pre-scan plus GetSignedTxinWeight -- so a copy here would
        // scale with inputs x solving-data for data that cannot change during the walk.
        // coin_control outlives `providers`, which is a local.
        providers.AddProvider(std::make_unique<BorrowedSigningProvider>(coin_control->m_external_provider));
    }
...
{
    // nVersion + nTime + nLockTime + input count + output count
                           ...
                           const CoinFilterParams& params)
{
    ...
    const bool can_grind_r = wallet.CanGrindR();
    // Evaluated once per call: the caller's solving data does not change while we walk
    // the wallet's outputs, so neither the predicate nor the provider belongs in the loop.
    const bool has_external_solving_data{coinControl && HasExternalSolvingData(coinControl->m_external_provider)};
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            // Automatic selection has to see solving_data too, or an output the caller just
            // supplied the keys for is still judged unsolvable and never selected. The merge
            // has to happen here because CalculateMaximumSignedInputSize infers the
            // descriptor from this provider argument alone; the coinControl it also receives
            // is consulted only by UseMaxSig, deciding whether to assume a maximum-size
            // signature, and never for key material. (Input-weight overrides are a different
            // path again -- GetSignedTxinWeight, not this one.)
            //
            // Guarded on there actually being external data: with none, `provider` is left
            // untouched, so every existing caller keeps byte-identical behaviour including
            // the case where the wallet returns none and the size call short-circuits to -1.
            //
            // Order matters and is relied on: MultiSigningProvider answers from the first
            // provider that succeeds (script/signingprovider.cpp -- every accessor returns
            // on first hit, none union), so adding the wallet first means the wallet still
            // wins for anything it already knows and external data only fills gaps.
            //
            // The external provider is borrowed, not copied: it is identical for every
            // candidate output and copying it per output would scale with the solving data
            // while holding cs_wallet.
            if (has_external_solving_data) {
                auto combined = std::make_unique<MultiSigningProvider>();
                if (provider) combined->AddProvider(std::move(provider));
                combined->AddProvider(std::make_unique<BorrowedSigningProvider>(coinControl->m_external_provider));
                provider = std::move(combined);
            }
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // Static vsize overhead + outputs vsize. TrollCoin adds a four-byte nTime
    // to the upstream 10-byte static transaction overhead.
    ...
    // The per-started-kB minimum fee is folded into the selection target here so the
    // selected value always covers it (the change-adjust below then settles the exact fee).
    ...
    // Blackcoin: and if we don't pay enough fees, reduce the change. Skipped for
    // subtract-fee-from-amount transactions: there the recipients pay the shortfall (the
    // guarded reduction below), never the change output.
    else if (nChangePosInOut != -1 && fee_needed > current_fee && !coin_selection_params.m_subtract_fee_outputs) {
        auto& change = txNew.vout.at(nChangePosInOut);
        const CAmount shortfall = fee_needed - current_fee;
        // The reduced change must stay spendable: a negative value is consensus-invalid
        // (bad-txns-vout-negative) and a dust value is unrelayable.
        if (IsDust(CTxOut(change.nValue - shortfall, change.scriptPubKey), wallet.chain().relayDustFee())) {
            return util::Error{_("The transaction change is too small to pay the fee")};
        }
