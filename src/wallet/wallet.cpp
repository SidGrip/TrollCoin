...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (!fInsertedNew)
    {
        if (state.index() != wtx.m_state.index()) {
            wtx.m_state = state;
            fUpdated = true;
        } else if (std::holds_alternative<TxStateInactive>(wtx.m_state) &&
                   std::holds_alternative<TxStateInactive>(state)) {
            // Two TxStateInactive values are one variant but serialize differently:
            // -1 when abandoned, 0 otherwise. AbandonTransaction stores abandoned=true
            // and a later reorg re-syncs the same tx through blockDisconnected as plain
            // TxStateInactive{}. Upstream asserts that pair cannot occur; at 64-second
            // spacing with staking it does, and the abort leaves the wallet permanently
            // unloadable -- the node dies on every start, before any RPC exists to
            // repair it.
            //
            // Merge instead of overwriting. A disconnect only says "no longer in a
            // block", never "the user did not abandon this", so abandonment must
            // survive it: clearing the flag would re-attach the tx and can re-lock
            // inputs the owner already freed for respend.
            const bool prev{std::get<TxStateInactive>(wtx.m_state).abandoned};
            const bool next{std::get<TxStateInactive>(state).abandoned};
            if (next && !prev) {
                wtx.m_state = TxStateInactive{/*abandoned=*/true};
                fUpdated = true;
            } else if (prev && !next) {
                WalletLogPrintf("AddToWallet: keeping abandoned=1 for %s (incoming inactive is not abandoned)\n",
                                hash.ToString());
            }
        } else {
            // Any other same-variant serialization mismatch is a real inconsistency,
            // not an expected transition. Keep the upstream pressure on it.
...
{
    // Witness change types require their soft fork to be active at the tip: a pre-activation
    // witness output is anyone-can-spend on this chain, so fall back to legacy in that window.
    const bool segwit_active{chain().isSegwitActive()};
    const bool taproot_active{chain().isTaprootActive()};

    // If -changetype is specified, honour it — unless it is a not-yet-active witness type.
    if (change_type) {
        if ((*change_type == OutputType::BECH32 || *change_type == OutputType::P2SH_SEGWIT) && !segwit_active) {
            return OutputType::LEGACY;
        }
        if (*change_type == OutputType::BECH32M && !taproot_active) {
            return OutputType::LEGACY;
        }
        return *change_type;
    }
    ...
    if (has_bech32m_spkman && any_tr && taproot_active) {
    ...
    if (has_bech32m_spkman && taproot_active) {
        return OutputType::BECH32M;
    }
...
{
    LOCK(cs_wallet);
    // Never vend a witness receive destination before its soft fork is active at the tip — a
    // pre-activation witness output is anyone-can-spend on this chain. Central guard so every
    // caller (RPC getnewaddress, the GUI receive page, internal callers) is covered.
    if (type == OutputType::BECH32 && !chain().isSegwitActive()) {
        return util::Error{_("bech32 (SegWit) addresses are not available until SegWit activates")};
    }
    if (type == OutputType::BECH32M && !chain().isTaprootActive()) {
        return util::Error{_("bech32m (Taproot) addresses are not available until Taproot activates")};
    }
    if (type == OutputType::P2SH_SEGWIT && !chain().isSegwitActive()) {
        return util::Error{_("p2sh-segwit addresses are not available until SegWit activates")};
    }
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart trollcoind with -walletcrosschain to override.");
                return false;
            }
        }
    }
