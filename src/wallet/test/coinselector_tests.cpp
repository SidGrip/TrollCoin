...
namespace wallet {
/** The spendable add_coin() helper below vends a BECH32 destination, and CWallet::GetNewDestination
 *  refuses witness types until their soft fork is active at the tip. SegWit/Taproot are parked at
 *  height INT_MAX on mainnet here (kernel/chainparams.cpp), so the default MAIN fixture can never
 *  hand one out; on regtest they are buried at height 1. Run the whole suite on regtest. */
struct CoinSelectorTestingSetup : public WalletTestingSetup {
    CoinSelectorTestingSetup() : WalletTestingSetup(ChainType::REGTEST) {}
};

BOOST_FIXTURE_TEST_SUITE(coinselector_tests, CoinSelectorTestingSetup)
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
    ...
    CoinSelectionParams dummy_params{ // Only used to provide the 'avoid_partial' flag.
            ...
            /*effective_feerate=*/CFeeRate(0),
            /*discard_feerate=*/CFeeRate(0),
            /*tx_noinputs_size=*/14 + 34, // TrollCoin static header (including nTime) + output size
            /*avoid_partial=*/false,
    };
...
static util::Result<SelectionResult> select_coins(const CAmount& target, const CoinSelectionParams& cs_params, const CCoinControl& cc, std::function<CoinsResult(CWallet&)> coin_setup, const node::NodeContext& m_node)
{
    ...
    if (result) {
        const auto signedTxSize = 14 + 34 + 68 * result->GetInputSet().size(); // TrollCoin static header (including nTime) + output size + inputs size (P2WPKH)
