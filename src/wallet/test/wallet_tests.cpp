...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        // Two coinbases scanned (oldTip at height 100 and newTip at 101), both still immature.
        // TrollCoin's PoW subsidy is 125 TROLL per block up to height 777,777, not Bitcoin's 50
        // (validation.cpp GetProofOfWorkSubsidy), so this is 2 x 125 rather than 2 x 50.
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, 250 * COIN);
    ...
    // The upstream test continues here with two more cases that prune the old and then the
    // remaining block file and expect the rescan to FAIL because the blocks are unreadable.
    // BlockManager has no PruneOneBlockFile/UnlinkPrunedFiles in this tree (see
    // node/blockstorage.h), so the pruned-block rescan-failure cases cannot be simulated and
    // both scans would simply succeed. Restore them alongside prune support.
}
...
BOOST_FIXTURE_TEST_CASE(importmulti_rescan, TestChain100Setup)
{
    ...
    */
    ...
    // after.
    {
        ...
        // Upstream expects the first key to fail with a "Rescan failed" error because the block
        // file holding oldTip was pruned above. That prune step is disabled here (BlockManager
        // has no PruneOneBlockFile/UnlinkPrunedFiles in this tree), so every block is still
        // readable and both keys import successfully. Restore alongside prune support.
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    wtx.MarkDirty();
    AddKey(wallet, coinbaseKey);
    // 125 TROLL, not Bitcoin's 50: the tip coinbase pays the PoW subsidy for heights up to
    // 777,777 (validation.cpp GetProofOfWorkSubsidy).
    BOOST_CHECK_EQUAL(CachedTxGetImmatureCredit(wallet, wtx, ISMINE_SPENDABLE), 125*COIN);
}
...
static int64_t AddTx(ChainstateManager& chainman, CWallet& wallet, uint32_t lockTime, int64_t mockTime, int64_t blockTime)
{
    ...
    // nTime is part of the txid on this chain and CMutableTransaction stamps it from the clock
    // (primitives/transaction.cpp), so two calls with the same lockTime would otherwise produce
    // two different transactions instead of updating the same wallet entry. Pin it.
    tx.nTime = 0;
...
// An abandoned transaction re-synced by a reorg arrives as plain TxStateInactive.
// The two states are one variant but serialize differently (-1 vs 0); upstream
// asserted that pair impossible and the abort left real wallets unloadable on a
// 64-second PoS chain. AddToWallet must merge instead: abandonment survives a
// non-abandoned resync, and a fresh abandon still lands.
BOOST_FIXTURE_TEST_CASE(inactive_state_merge_keeps_abandoned, TestingSetup)
{
    CWallet wallet(m_node.chain.get(), "", CreateMockableWalletDatabase());
    CMutableTransaction mtx;
    mtx.vout.resize(1);
    ...
    // abandoned first, then the reorg-shaped resync: previously an abort
    wallet.AddToWallet(tx, TxStateInactive{/*abandoned=*/true});
    const CWalletTx* wtx = wallet.AddToWallet(tx, TxStateInactive{});
    BOOST_REQUIRE(wtx);
    BOOST_CHECK(wtx->isAbandoned());

    // the reverse order must still adopt the abandon
    CMutableTransaction mtx2{mtx};
    mtx2.vout[0].nValue = 2 * COIN;
    const CTransactionRef tx2 = MakeTransactionRef(mtx2);
    wallet.AddToWallet(tx2, TxStateInactive{});
    const CWalletTx* wtx2 = wallet.AddToWallet(tx2, TxStateInactive{/*abandoned=*/true});
    BOOST_REQUIRE(wtx2);
    BOOST_CHECK(wtx2->isAbandoned());
}
...
public:
    ...
    //! Number of coinbases the wallet can already spend at the current tip. Regtest
    //! nCoinbaseMaturity is 10 here (kernel/chainparams.cpp), not Bitcoin's 100, while
    //! TestChain100Setup still mines 100 blocks, so "exactly one mature coinbase" no longer
    //! holds. A coinbase at height h is spendable once its depth reaches nCoinbaseMaturity + 1
    //! (CWallet::GetTxBlocksToMaturity), i.e. once h <= tip - nCoinbaseMaturity.
    size_t MatureCoinbaseCount()
    {
        const int tip_height{WITH_LOCK(Assert(m_node.chainman)->GetMutex(), return m_node.chainman->ActiveChain().Height())};
        return static_cast<size_t>(tip_height - Assert(m_node.chainman)->GetParams().GetConsensus().nCoinbaseMaturity);
    }

    //! Total value of those mature coinbases, from the emission schedule in validation.cpp
    //! GetProofOfWorkSubsidy: block 1 mints the 500,000,000 TROLL v1 swap allocation and every
    //! later PoW block up to height 777,777 mints 125 TROLL.
    CAmount MatureCoinbaseTotal()
    {
        return 500000000 * COIN + static_cast<CAmount>(MatureCoinbaseCount() - 1) * 125 * COIN;
    }
...
BOOST_FIXTURE_TEST_CASE(ListCoinsTest, ListCoinsTestingSetup)
{
    ...
    // Upstream assumes exactly one mature coinbase here; this chain matures them after
    // nCoinbaseMaturity + 1 confirmations, so derive the count instead of hardcoding 1.
    const size_t mature{MatureCoinbaseCount()};

    // Confirm ListCoins initially returns the mature coins grouped under the
    // coinbaseKey address.
    ...
    BOOST_CHECK_EQUAL(list.begin()->second.size(), mature);

    // Check initial balance from the mature coinbase transactions.
    BOOST_CHECK_EQUAL(MatureCoinbaseTotal(), WITH_LOCK(wallet->cs_wallet, return AvailableCoins(*wallet).GetTotalAmount()));
    ...
    // AddTx mined a block, maturing one more coinbase, and funded itself from one mature
    // coinbase, so the coinbase count is unchanged and the change coin is the extra one.
    BOOST_CHECK_EQUAL(list.size(), 1U);
    BOOST_CHECK_EQUAL(std::get<PKHash>(list.begin()->first).ToString(), coinbaseAddress);
    BOOST_CHECK_EQUAL(list.begin()->second.size(), mature + 1);

    // Lock every coin. Confirm number of available coins drops to 0.
    {
        LOCK(wallet->cs_wallet);
        BOOST_CHECK_EQUAL(AvailableCoinsListUnspent(*wallet).Size(), mature + 1);
    }
...
BOOST_FIXTURE_TEST_CASE(BasicOutputTypesTest, ListCoinsTest)
{
    ...
    // DEFAULT_ADDRESS_TYPE is LEGACY on this chain (wallet.h) because SegWit is parked on
    // mainnet, and CWallet::TransactionChangeType then returns LEGACY for every change output,
    // which would break the "change matches the recipient type" premise below. This fixture runs
    // on regtest, where SegWit and Taproot are active from height 1, so restore the upstream
    // premise for this test alone. Do not change DEFAULT_ADDRESS_TYPE itself.
    ...
    expected_coins_sizes[OutputType::UNKNOWN] = MatureCoinbaseCount();
    ...
    //
    // The Other bucket stays at its starting size throughout: every self transfer mines one
    // block, maturing one more coinbase, and spends exactly one mature coinbase to fund itself.
 ...
 */
// Regtest, not the default MAIN: the test needs a BECH32M destination below, and
// CWallet::GetNewDestination refuses witness types until their soft fork is active at the tip.
// SegWit/Taproot are parked at height INT_MAX on mainnet here (kernel/chainparams.cpp); on
// regtest they are buried at height 1.
BOOST_FIXTURE_TEST_CASE(wallet_sync_tx_invalid_state_test, RegTestingSetup)
{
    CWallet wallet(m_node.chain.get(), "", CreateMockableWalletDatabase());
    {
        LOCK(wallet.cs_wallet);
