...
void OutputGroup::Insert(const std::shared_ptr<COutput>& output, size_t ancestors, size_t descendants) {
    ...
    // Flat-fee chain: no fee market to defer to, so future spends are not discounted.
    // A zero long-term fee makes each extra input count as waste, keeping selection minimal.
    coin.long_term_fee = 0;
