...
uint64_t GetStakeWeight(const CWallet& wallet)
{
    ...
    {
        // Only coins past nStakeMinConfirmations can actually stake (CheckKernel in
        // pos.cpp gates on it), so weight must use it — not the shorter nCoinbaseMaturity —
        // or the wallet reports stake weight for coins it cannot yet stake.
        if (wallet.GetTxDepthInMainChain(*pcoin.first) >= Params().GetConsensus().nStakeMinConfirmations)
        {
                           ...
                           const CoinFilterParams& params)
{
    ...
    // Stake maturity is nStakeMinConfirmations (consensus gate in CheckKernel, pos.cpp), not
    // the shorter nCoinbaseMaturity — selecting coins below it makes the staker grind coins it
    // cannot stake ("CheckKernel(): Coin is not mature") and inflates the displayed stake weight.
    const int min_depth = std::max(DEFAULT_MIN_DEPTH, Params().GetConsensus().nStakeMinConfirmations);
...
bool CreateCoinStake(CWallet& wallet, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, CAmount& nFees, CTxDestination destination)
{
    ...
    {
        // The wallet already holds the full previous transaction for each of its own coins,
        // so use it directly for coinstake signing (vwtxPrev) instead of a txindex lookup —
        // staking does not require -txindex. The kernel check reads value/height/time from the
        // UTXO view, and descriptor wallets sign from findCoins, so neither needs txindex either.
        const CTransactionRef& tx = pcoin.first->tx;
    ...
    // TrollCoin has no dev-fund / stake-reward donation: the staker keeps the full reward.
    nCredit += nReward;
