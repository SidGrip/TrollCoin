...
RPCHelpMan getnewaddress()
{
    return RPCHelpMan{"getnewaddress",
                ...
                RPCResult{
                    RPCResult::Type::STR, "address", "The new trollcoin address"
                },
                RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Before their soft fork activates at the tip a witness output is anyone-can-spend on this
    // chain — withhold bech32/bech32m for BOTH an explicit request and the -addresstype default.
    if (output_type == OutputType::BECH32 && !pwallet->chain().isSegwitActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "bech32 (SegWit) addresses are not available until SegWit activates");
    }
    if (output_type == OutputType::BECH32M && !pwallet->chain().isTaprootActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "bech32m (Taproot) addresses are not available until Taproot activates");
    }
    if (output_type == OutputType::P2SH_SEGWIT && !pwallet->chain().isSegwitActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "p2sh-segwit addresses are not available until SegWit activates");
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Withhold witness change outputs until their soft fork is active at the tip (explicit or
    // -changetype default) — a pre-activation witness output is anyone-can-spend on this chain.
    if (output_type == OutputType::BECH32 && !pwallet->chain().isSegwitActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "bech32 (SegWit) change addresses are not available until SegWit activates");
    }
    if (output_type == OutputType::BECH32M && !pwallet->chain().isTaprootActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "bech32m (Taproot) change addresses are not available until Taproot activates");
    }
    if (output_type == OutputType::P2SH_SEGWIT && !pwallet->chain().isSegwitActive()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "p2sh-segwit change addresses are not available until SegWit activates");
    }
...
RPCHelpMan setlabel()
{
    return RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The trollcoin address to be associated with a label."},
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::ARR, "", "",
                        {
                            ...
                            {
                                {RPCResult::Type::STR, "address", "The trollcoin address"},
...
RPCHelpMan addmultisigaddress()
{
    return RPCHelpMan{"addmultisigaddress",
                ...
                "Note: This command is only compatible with legacy wallets.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The trollcoin addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "trollcoin address or hex-encoded public key"},
                        },
                        },
...
RPCHelpMan getaddressinfo()
{
    return RPCHelpMan{"getaddressinfo",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The trollcoin address for which to get information."},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The trollcoin address validated."},
...
RPCHelpMan walletdisplayaddress()
{
    return RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "trollcoin address to display"},
        },
        RPCResult{
            RPCResult::Type::OBJ,"","",
            {
