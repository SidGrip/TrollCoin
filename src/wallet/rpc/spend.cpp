...
static void ParseRecipients(const UniValue& address_amounts, const UniValue& subtract_fee_outputs, std::vector<CRecipient>& recipients)
{
    ...
    for (const std::string& address: address_amounts.getKeys()) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TrollCoin address: ") + address);
        }
...
RPCHelpMan optimizeutxoset()
{
    return RPCHelpMan{"optimizeutxoset",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The trollcoin address to recieve all the new UTXOs. If not provided, new UTOXs will be assigned to the address of the input UTXOs."},
                    ...
                    {"fromAddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "The trollcoin address to split coins from. If not provided, all available coins will be used."},
                },
                {
                ...
                RPCExamples{
                    "\nTrigger UTXO optimization and assign all the new UTXOs to some trollcoin address with user defined UTXO value\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[3].isNull() == false) {
        ...
        if (!IsValidDestination(fromAddress)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TrollCoin address: ") + request.params[3].get_str());
        }
...
RPCHelpMan sendtoaddress()
{
    return RPCHelpMan{"sendtoaddress",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The trollcoin address to send to."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // fee_rate is a floor to raise, never a discount: CreateTransaction takes the
    // greater of this rate and the flat per-started-kB minimum, so a rate below
    // the floor changes nothing rather than producing an underpaying transaction.
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The trollcoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // fee_rate is a floor to raise, never a discount: CreateTransaction takes the
    // greater of this rate and the flat per-started-kB minimum.
...
//! Turn a solving_data options object into external keys and scripts for coin control.
//!
//! Shared because two callers need it and only one had it. FundTransaction parsed this
//! inline, so sendall -- which builds its transaction itself and never calls
//! FundTransaction -- accepted solving_data and silently dropped it, leaving the wallet
//! to size watch-only inputs without the keys it had just been handed. Extracted rather
//! than duplicated so the two paths cannot drift apart the way fee_rate did.
static void ApplySolvingData(const UniValue& options, CCoinControl& coinControl)
{
...
void FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, const UniValue& options, CCoinControl& coinControl, bool override_min_fee)
{
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        // fundrawtransaction, send and walletcreatefundedpsbt all declare minconf and
        // maxconf and document them, but the strict key check above rejected both --
        // "Unexpected key minconf" -- so every caller that followed the help text got an
        // error. sendall was the only call that read them at all. Nothing was silently
        // mis-selected: the constraint never reached coin selection because the call never
        // got that far. Wallet-side only; m_min_depth and m_max_depth are read by
        // AvailableCoins (wallet/spend.cpp) when choosing which of our own UTXOs to spend,
        // and no validation rule consults them.
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid trollcoin address");
            }
        ...
        if (options.exists("feeRate")) {
            ...
            // Only the deprecated camelCase spelling was ever read here, and
            // PreventOutdatedOptions rejects that spelling outright for send and
            // sendall, so fee_rate was documented and then discarded on every
            // call that funds a transaction. The rate is a floor to raise, not a
            // price: CreateTransaction already takes the greater of the
            // requested rate and the flat per-started-kB minimum.
    ...
    ApplySolvingData(options, coinControl);
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "For backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The trollcoin address to receive the change"},
...
static std::vector<RPCArg> OutputsDoc()
{
    return
    {
        {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
            {
                {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the trollcoin address,\n"
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_address", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The trollcoin address to receive the change"},
    ...
    return RPCHelpMan{"sendall",
        ...
        "If your wallet contains many small inputs, either because it received tiny payments or as a result of accumulating change, consider using `send_max` to exclude inputs that are worth less than the fees needed to spend them.\n",
        {
            {"recipients", RPCArg::Type::ARR, RPCArg::Optional::NO, "The sendall destinations. Each address may only appear once.\n"
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "A trollcoin address which receives an equal share of the unspecified amount."},
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the trollcoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                    },
                },
            },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // solving_data has to reach three places to mean anything, and parsing it only
            // reached the first: the provider here, coin selection in AvailableCoins, and
            // whole-transaction sizing below. Wiring only the parse made sendall reject a
            // malformed key while a well-formed one changed nothing.
            ApplySolvingData(options, coin_control);

            // sendall builds its transaction here rather than through FundTransaction, so it
            // needs its own read of fee_rate; wiring the other callers left this one still
            // discarding the argument it documents. Deliberately without fOverrideFeeRate:
            // that flag makes GetMinimumFeeRate return the requested rate untouched, which
            // would render the check just below vacuous. Leaving it off lets a rate under the
            // wallet minimum be caught and refused, which is what that check is for and what
            // a call that sweeps the entire balance should do rather than quietly pay more
            // than was asked.
            if (options.exists("fee_rate")) {
            ...
            if (options.exists("inputs") && options.exists("send_max")) {
                ...
                for (const COutput& output : AvailableCoins(*pwallet, &coin_control, fee_rate, coins_params).All()) {
                    // A watch-only wallet with include_watching and no solving_data is a
                    // legitimate state, and AvailableCoins returns such an output as
                    // spendable-but-unsolvable with input_bytes == -1. Asserting here turned
                    // "you did not give me the key" into an internal-bug message. Refuse it
                    // the same way, and with the same words, as the sizing check below.
            ...
            // estimate final size of tx. Coin control must be passed: GetDescriptor merges
            // coin_control.m_external_provider only when it is supplied, so without it any
            // solving_data the caller gave is ignored at sizing.
            const TxSize tx_size{CalculateMaximumSignedTxSize(CTransaction(rawTx), pwallet.get(), &coin_control)};
            if (tx_size.vsize < 0 || tx_size.weight < 0) {
                // Without this, a -1 flows into the fee arithmetic and surfaces as a
                // maximum-fee complaint, which says nothing about the actual problem.
                throw JSONRPCError(RPC_WALLET_ERROR, "Missing solving data for estimating transaction size");
            }
