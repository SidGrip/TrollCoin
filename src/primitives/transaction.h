...
void UnserializeTransaction(TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s >> tx.nVersion;
    s >> tx.nTime;  // TrollCoin carries nTime on every tx version
    unsigned char flags = 0;
    tx.vin.clear();
    tx.vout.clear();
...
{
public:
    // Default transaction version. TrollCoin transactions are version 1; a v1 tx serializes
    // nTime in both the wire and signature-hash forms, which legacy nodes require to verify.
