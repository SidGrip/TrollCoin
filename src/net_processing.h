...
/** Maximum size of the list of indexes in the header spam filter.
 *  Raised from 500 to match upstream: fixing the bucket key means the
 *  filter does real per-peer accounting for the first time, so the
 *  threshold has to suit a busy proof-of-stake network or honest peers
 *  trip it. The two changes belong together. */
