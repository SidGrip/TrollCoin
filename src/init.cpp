 ...
 */
static const char* BITCOIN_PID_FILENAME = "trollcoind.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-parallelheaderchecks=<n>", strprintf("Set the number of threads used to pre-compute block-header scrypt hashes in parallel during headers sync (0 = auto up to %d, 1 = serial/off, <0 = leave that many cores free, default: %d)",
        MAX_PARALLEL_HEADER_CHECKS, DEFAULT_PARALLEL_HEADER_CHECKS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (chain == ChainType::SIGNET) {
        if (!args.IsArgSet("-signetchallenge")) {
            return InitError(_("TrollCoin has no default signet network. To run a custom signed test network, restart with -signetchallenge=<hex>."));
        }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if trollcoind is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if trollcoind is started while in a temporary directory.\n",
    ...
    // Header-hash pre-warm threads: -par-style (0=auto, -n=leave-free), capped, min 1.
    int header_check_threads = args.GetIntArg("-parallelheaderchecks", DEFAULT_PARALLEL_HEADER_CHECKS);
    if (header_check_threads <= 0) {
        header_check_threads += GetNumCores();
    }
    header_check_threads = std::max(header_check_threads, 1);
    header_check_threads = std::min(header_check_threads, MAX_PARALLEL_HEADER_CHECKS);
    g_parallel_header_check_threads = header_check_threads;
    LogPrintf("Header-sync hash pre-warming uses %d threads\n", header_check_threads);
    ...
    */
    ...
    // Advertise a fixed "3.0" version label in the user agent (/Trolloshi:3.0/); keep -uacomment support.
    strSubVersion = "/" + CLIENT_NAME + ":3.0";
    if (!uacomments.empty()) {
        strSubVersion += "(" + uacomments[0];
        for (size_t i = 1; i < uacomments.size(); ++i)
            strSubVersion += "; " + uacomments[i];
        strSubVersion += ")";
    }
    strSubVersion += "/";
