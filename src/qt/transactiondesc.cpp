    ...
    else
    {
        ...
        if (nNet > 0)
        {
            ...
            if (IsValidDestination(address)) {
                ...
                {
                    // Recover the sender address(es) from the inputs. TrollCoin coins
                    // are usually P2PK, whose scriptSig is only the signature, so the
                    // pubkey lives in the prevout. Resolve each prevout's scriptPubKey
                    // first from the wallet (own coins), then via -txindex (any tx), and
                    // finally fall back to a P2PKH scriptSig pubkey when the prevout tx
                    // is unavailable (no txindex and not ours).
                    std::set<std::string> fromAddresses;
                    auto add_prevout_address = [&fromAddresses](const CTransactionRef& prevTx, uint32_t n) {
                        if (!prevTx || n >= prevTx->vout.size()) return false;
                        CTxDestination dest;
                        if (!ExtractDestination(prevTx->vout[n].scriptPubKey, dest)) return false;
                        fromAddresses.insert(EncodeDestination(dest));
                        return true;
                    };
                    // Cap the expensive -txindex (disk) lookups so a tx with many
                    // third-party inputs cannot stall the details popup; the in-wallet
                    // and scriptSig paths are cheap and stay uncapped.
                    constexpr int MAX_TXINDEX_LOOKUPS = 50;
                    int txindexLookups = 0;
                    bool moreInputs = false;
                    for (const CTxIn& txin : wtx.tx->vin) {
                        if (add_prevout_address(wallet.getTx(txin.prevout.hash), txin.prevout.n)) continue;
                        if (txindexLookups < MAX_TXINDEX_LOOKUPS) {
                            ++txindexLookups;
                            if (add_prevout_address(node.getTransaction(txin.prevout.hash), txin.prevout.n)) continue;
                        } else {
                            moreInputs = true;
                        }
                    ...
                    if (moreInputs && !fromAddresses.empty()) fromAddresses.insert("…");
