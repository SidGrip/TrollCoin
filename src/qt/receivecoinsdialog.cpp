...
void ReceiveCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        // Offer witness receive-address types only once their soft fork is active at the
        // tip (regtest, or after the buried flag day). GetNewDestination's central guard
        // refuses to vend a witness address before then regardless of this dropdown.
        if (model->wallet().isSegwitActive()) {
            ...
            if (model->wallet().isTaprootActive() && model->wallet().taprootEnabled()) {
        ...
        // Once SegWit is active, default the selection to Bech32 (modern standard, lower fees
        // than legacy); Bech32m/Taproot stays opt-in as payer support is still limited. An
        // explicit non-legacy -addresstype was already selected by add_address_type above.
        if (model->wallet().getDefaultAddressType() == OutputType::LEGACY && model->wallet().isSegwitActive()) {
            const int bech32_index = ui->addressType->findData((int) OutputType::BECH32);
            if (bech32_index != -1) ui->addressType->setCurrentIndex(bech32_index);
        }
