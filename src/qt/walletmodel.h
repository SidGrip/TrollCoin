...
public:
    ...
    {
    public:
        UnlockContext(WalletModel *wallet, bool valid, bool relock, bool restoreStaking, bool prevEnabledStaking);
        ~UnlockContext();
    ...
    private:
        WalletModel *wallet;
        const bool valid;
        const bool relock;             // fully re-lock the wallet when the context ends
        const bool restoreStaking;     // return the wallet to staking-only unlock when the context ends
        const bool prevEnabledStaking; // staking-armed state to restore after a locked-wallet send
    };
