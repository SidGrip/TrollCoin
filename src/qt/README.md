This directory contains the source code for the TrollCoin graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.

The current precise version for Qt 5 is specified in [qt.mk](/depends/packages/qt.mk).

## Compile and run

See build instructions: [Unix](/doc/build-unix.md), [macOS](/doc/build-osx.md), [Windows](/doc/build-windows.md)
...
- Contains translations inherited from upstream. Translations are compiled into the GUI at build time.
...
**Note:** Do not edit `locale/bitcoin_en.ts` by hand; it is regenerated from the source strings.
...
4. Enter `trollcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `trollcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `trollcoin-qt`
9. You're all set! Start developing, building, and debugging the TrollCoin GUI
