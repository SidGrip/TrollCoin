...
{
    this->model = _model;
    // Only the explicit staking unlock uses the checkbox; a send-unlock (Unlock mode)
    // must never inherit the persisted staking-only flag.
        ...
        try {
            if (!model->setWalletLocked(false, oldpass)) {
                ...
                if (UnlockStaking == mode) {
                    // Explicit staking unlock: set the persisted staking-only flag and arm staking.
                    ...
                    // Send-unlock (Unlock mode): clear the staking-only spend-gate so the
                    // send can sign. This doubles as the success signal WalletModel::
                    // requestUnlock() reads (a cancel leaves the flag set → invalid). On a
                    // staking-only wallet the keystore was never re-locked, so staking is
                    // uninterrupted; the UnlockContext dtor restores the staking-only unlock.
