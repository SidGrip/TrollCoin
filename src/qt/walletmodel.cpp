...
{
    // Key the post-operation behaviour off the ACTUAL lock state, not the persisted
    // staking-only flag (which survives a lock and can be stale). A fully-locked wallet
    // must re-lock after the operation even if that flag is stale; only a wallet that is
    // genuinely staking-unlocked keeps staking.
    const bool originally_locked = getEncryptionStatus() == Locked;
    const bool was_staking = (!originally_locked) && getWalletUnlockStakingOnly();

    // A locked wallet is briefly unlocked only to sign the send; suppress staking for
    // that window so a send-only unlock never starts the staker. The dtor restores the
    // prior armed state (after re-lock the staker is paused by IsLocked() regardless).
    bool prev_enabled_staking = false;
    if (originally_locked)
    {
        prev_enabled_staking = m_wallet->getEnabledStaking();
        m_wallet->setEnabledStaking(false);
    }

    if (originally_locked || was_staking)
    {
        // A staking-only wallet is re-authenticated IN PLACE — no re-lock — so the
        // keystore stays unlocked and staking never pauses. The passphrase dialog
        // verifies the passphrase via Unlock() (which never clears the master key) and
        // clears the staking-only flag on success.
        Q_EMIT requireUnlock();
    }

    // Success signal: a re-authenticated staking-only wallet has had its staking-only
    // flag cleared by the dialog; a locked wallet is now unlocked. Cancel leaves the
    // flag set / the wallet locked, so valid == false and the send aborts.
    const bool valid = was_staking ? !getWalletUnlockStakingOnly()
                                    : (getEncryptionStatus() != Locked);

    // relock: fully re-lock after the op (wallet was locked to begin with).
    // restoreStaking: return to staking-only unlock (wallet was staking to begin with).
    return UnlockContext(this, valid, originally_locked, was_staking, prev_enabled_staking);
}

WalletModel::UnlockContext::UnlockContext(WalletModel *_wallet, bool _valid, bool _relock, bool _restoreStaking, bool _prevEnabledStaking):
        wallet(_wallet),
        valid(_valid),
        relock(_relock),
        restoreStaking(_restoreStaking),
        prevEnabledStaking(_prevEnabledStaking)
{
    // The operation (e.g. a send) needs the staking-only spend-gate cleared to sign.
    // The dtor restores the correct final state.
    if (valid) wallet->setWalletUnlockStakingOnly(false);
}
...
WalletModel::UnlockContext::~UnlockContext()
{
    if (relock)
    {
        // Wallet was locked for the send: re-lock (only if we actually unlocked it) and
        // restore the staking-armed state suppressed for the unlock window. Runs on
        // cancel too — the send aborted and the wallet is still locked.
        if (valid) wallet->setWalletLocked(true);
        wallet->wallet().setEnabledStaking(prevEnabledStaking);
        wallet->updateStatus();
        return;
    }
    ...
    if (restoreStaking)
    {
        wallet->setWalletUnlockStakingOnly(true); // restore staking-only unlock
    }
    wallet->updateStatus();
}
