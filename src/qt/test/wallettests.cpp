...
void VerifyUseAvailableBalance(SendCoinsDialog& sendCoinsDialog, const WalletModel& walletModel)
{
    ...
    // A selected-coin sweep uses subtract-fee-from-amount, so its preview has
    // two 148-byte legacy inputs, one recipient output, and TrollCoin's
    // 14-byte static transaction overhead (including nTime).
    QVERIFY(send_entry->getValue().fSubtractFeeFromAmount);
    sendCoinsDialog.updateTabsAndLabels();
    QLabel* bytes_label = sendCoinsDialog.findChild<QLabel*>("labelCoinControlBytes");
    QVERIFY(bytes_label);
    QCOMPARE(bytes_label->text(), QString::fromUtf8(ASYMP_UTF8) + QStringLiteral("344"));
}
...
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("trollcoin:"), 2);
