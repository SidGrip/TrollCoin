...
void WalletView::processNewTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    // Suppress the reopen / catch-up backlog: as the node connects blocks that piled up
    // while the GUI was closed, each wallet transaction is replayed and would toast a
    // separate balloon (a day of staking mints = a storm). IBD alone doesn't cover this —
    // IBD clears once the tip is within ~a day. A transaction whose wallet time is not
    // recent is backlog, not a live payment, so skip its notification.
    static constexpr qint64 NOTIFY_MAX_AGE_SECS = 300; // ~5 blocks at 64 s spacing
    const QDateTime txDate = index.data(TransactionTableModel::DateRole).toDateTime();
    if (txDate.isValid() && txDate.secsTo(QDateTime::currentDateTime()) > NOTIFY_MAX_AGE_SECS) {
        return;
    }
...
void WalletView::unlockWallet(bool fromMenu)
{
    ...
    const bool locked = walletModel->getEncryptionStatus() == WalletModel::Locked;
    // A send from a staking-only-unlocked wallet must re-authenticate to spend, but the
    // keystore is already unlocked — prompt in "Unlock" mode so the dialog re-verifies
    // the passphrase in place (no re-lock) and staking never pauses.
    const bool staking_send = !locked && !fromMenu && walletModel->getWalletUnlockStakingOnly();
    if (locked || staking_send)
    {
        AskPassphraseDialog::Mode mode = (locked && fromMenu) ?
...
void WalletView::lockWallet()
{
    ...
    walletModel->wallet().setEnabledStaking(false); // explicit lock stops staking
}
