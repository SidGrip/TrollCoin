...
namespace {
constexpr unsigned int COIN_CONTROL_TX_BASE_BYTES{
    4 /* nVersion */ +
    4 /* TrollCoin nTime */ +
    1 /* input count */ +
    1 /* output count */ +
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    {
        // Bytes
        // Always assume one extra output for change.
        nBytes = nBytesInputs +
                 ((CoinControlDialog::payAmounts.size() > 0 ? CoinControlDialog::payAmounts.size() + 1 : 2) * 34) +
                 COIN_CONTROL_TX_BASE_BYTES;
        if (fWitness)
        {
