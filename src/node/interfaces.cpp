...
public:
    ...
    CTransactionRef getTransaction(const uint256& hash) override
    {
        // Resolve a transaction by id via -txindex; GetTransaction returns null when
        // txindex is off. block_index and mempool are null, so only the self-locking
        // txindex path runs and no cs_main is needed here.
        uint256 hash_block;
        return node::GetTransaction(/*block_index=*/nullptr, /*mempool=*/nullptr, hash, hash_block, chainman().m_blockman);
    }
...
public:
    ...
    bool isSegwitActive() const override
    {
        LOCK(::cs_main);
        const CBlockIndex* tip = Assert(m_node.chainman)->ActiveChain().Tip();
        return DeploymentActiveAfter(tip, *m_node.chainman, Consensus::DEPLOYMENT_SEGWIT);
    }
