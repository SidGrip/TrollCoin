...
int64_t UpdateTime(CBlock* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    pblock->InvalidateCache(); // nTime/nBits may have changed -> cached hash is stale
...
void RegenerateCommitments(CBlock& block, ChainstateManager& chainman)
{
    ...
    // Pre-activation (and for any commitment-less template) the coinbase carries no witness
    // commitment, so GetWitnessCommitmentIndex returns NO_WITNESS_COMMITMENT (-1); erasing at
    // begin()+(-1) corrupts the heap. Only erase when a commitment is actually present.
    const int commitment_index = GetWitnessCommitmentIndex(block);
    if (commitment_index != NO_WITNESS_COMMITMENT) {
        tx.vout.erase(tx.vout.begin() + commitment_index);
    }
    ...
    block.InvalidateCache(); // merkle root changed -> cached scrypt hash is stale
}
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool* pfPoSCancel, int64_t* pFees, CTxDestination destination)
{
    ...
    // TrollCoin blocks are always version 1; validation rejects nVersion > 1
    // (CheckBlockHeader). Do not stamp Core's versionbits value here.
    pblock->nVersion = 1;
    ...
    // Use the block template's captured time instead of taking a second wall
    // clock sample in the transaction constructor.
...
void IncrementExtraNonce(CBlock* pblock, const CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    pblock->InvalidateCache(); // merkle root changed -> cached scrypt hash is stale
}
...
static bool ProcessBlockFound(const CBlock* pblock, ChainstateManager& chainman)
{
    ...
    // Found a solution
    {
        LOCK(cs_main);
        BlockValidationState state;
        if (!CheckProofOfStake(&chainman.BlockIndex()[pblock->hashPrevBlock], *pblock->vtx[1], pblock->nBits, state, chainman.ActiveChainstate().CoinsTip(), pblock->vtx[1]->nTime))
...
void PoSMiner(CWallet *pwallet)
{
    ...
    util::ThreadRename(strprintf("trollcoin-stake-miner-%s", pwallet->GetName()));
