...
//! How far back the rolling sync-checkpoint sits, in blocks.
//!
//! This must equal the legacy client's value. Legacy TrollCoin uses
//! nCheckpointSpan = 500 (src/checkpoints.cpp) and enforces it in AcceptBlock,
//! so it accepts a fork reaching up to 500 blocks back. This tree previously
//! derived the span from nCoinbaseMaturity, which is 77 on mainnet, making it
//! the stricter node: a reorg deeper than ~77 blocks would be accepted by the
//! legacy majority and rejected here as bad-fork-prior-to-synch-checkpoint --
//! a hard BLOCK_INVALID_HEADER that also scores the peer 100. That is a chain
//! split, and the legacy nodes are the majority, so they are the reference.
//!
//! It is a plain constant rather than a chain parameter because the legacy
//! value is one constant for every network. On short chains the walk simply
//! stops at genesis, which is more permissive than the old 77, never less.
static constexpr int SYNC_CHECKPOINT_SPAN{500};
...
const CBlockIndex* BlockManager::AutoSelectSyncCheckpoint(const CBlockIndex *pindexBest)
{
    ...
    int checkpointSpan = SYNC_CHECKPOINT_SPAN;
