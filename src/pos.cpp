...
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, uint32_t blockFromTime, CAmount prevoutValue, const COutPoint& prevout, unsigned int nTimeTx, bool fPrintProofOfStake)
{
    ...
    // Weighted target. TrollCoin multiplies with unbounded CBigNum, so when
    // bnTarget*nValue exceeds 2^256 the kernel is always satisfied (any 256-bit
    // hash is below it). Detect that overflow rather than letting arith_uint256
    // wrap, which would wrongly reject low-difficulty (early) stakes.
    int64_t nValueIn = prevoutValue;
    if (nValueIn == 0)
        return error("CheckStakeKernelHash() : nValueIn = 0");
    arith_uint256 bnWeight = arith_uint256(nValueIn);
    bool fTargetOverflow = bnTarget > (~arith_uint256(0) / bnWeight);
    if (!fTargetOverflow)
        bnTarget *= bnWeight;
    ...
    if (!fTargetOverflow && UintToArith256(hashProofOfStake) > bnTarget)
        return false;
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, BlockValidationState& state, CCoinsViewCache& view, unsigned int nTimeTx)
{
    ...
    // Kernel (input 0) must meet the stake hash target (nBits). The kernel is
    // weighted by the prevout VALUE, not coin age (see CheckStakeKernelHash).
    ...
    // Stake maturity: the prevout must be at least nStakeMinConfirmations (777)
    // blocks deep. This is a confirmation-DEPTH gate, not an elapsed-time check
    // — do not reintroduce a time-based min-age here (it would fork off the
    // legacy network, where the 1h nStakeMinAge was wallet-policy only).
    if (pindexPrev->nHeight + 1 - coinPrev.nHeight < Params().GetConsensus().nStakeMinConfirmations){
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "stake-prevout-not-mature", strprintf("CheckProofOfStake() : Stake prevout is not mature, expecting %i and only matured to %i", Params().GetConsensus().nStakeMinConfirmations, pindexPrev->nHeight + 1 - coinPrev.nHeight));
    }
    ...
    // Verify signature
    // Keep the preliminary stake check aligned with the historically proven
    // validation path. Mandatory block script flags are applied later.
