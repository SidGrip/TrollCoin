...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <trollcoin-cli-call ...>'
  echo
  ...
  echo 'Examples:'
  echo
  echo "  ./contrib/devtools/utxo_snapshot.sh 4200000 utxo.dat ./src/trollcoin-cli -datadir=\$(pwd)/testdata"
  echo '  ./contrib/devtools/utxo_snapshot.sh 4200000 - ./src/trollcoin-cli'
  exit 1
fi
