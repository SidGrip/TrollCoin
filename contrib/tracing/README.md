...
Tracing (USDT) support for TrollCoin on Linux. For more information on
USDT support in TrollCoin see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `trollcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `trollcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[USDT documentation](../../doc/tracing.md#listing-available-tracepoints) on how
to list available tracepoints in your `trollcoind` binary.
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/trollcoind
```
...
P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/trollcoind
```
...
between height 20000 and 38000 while logging all blocks that take longer than
25ms to connect.
...
In a different terminal, starting trollcoind with re-indexing enabled.

```
$ ./src/trollcoind -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/trollcoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/trollcoind
```
