...
## TrollCoin 3.0 (2026-08-24)
- Rebased the original TrollCoin chain onto Bitcoin Core 26.2 (via Blackcoin More) — a consensus-identical drop-in on the live network (`/Trolloshi:3.0/`); proven by a full from-genesis resync with 0 rejects.
- Transaction fee matched byte-for-byte to the original TrollCoin 2.x network (flat per-started-kB minimum, 0.0001/kB).
- CSV / SegWit / Taproot / strict-DER (BIP66) are implemented but **parked (inactive) on mainnet**; active on testnet and regtest for development.
- Descriptor (HD) wallets by default; multi-wallet independent staking; header-sync acceleration.
- `-signet` no longer has a default network; running one requires an explicit `-signetchallenge`.
- Removed Android support and the MSVC project files, neither of which is built or tested.
- Translations rebranded to TrollCoin.


---

The entries below are the inherited **Blackcoin More** upstream changelog. Their
dates, activation schedules and coin references (BLK) are Blackcoin's — **not**
TrollCoin's; TrollCoin parks the soft forks described above.
